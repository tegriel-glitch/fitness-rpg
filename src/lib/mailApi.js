...
export async function dbLoadMessages(nickname) {
  try {
    ...
    if (!res.ok) {
      console.error('dbLoadMessages failed:', nickname, res.status, await res.text().catch(() => ''));
      return [];
    }
    return await res.json();
  } catch (e) {
    console.error('dbLoadMessages exception:', nickname, e.message);
    return [];
  }
}
export async function dbSendMessage(from, to, text, fromDisplayName, systemType = null) {
  try {
    ...
    if (!res.ok) {
      console.error('dbSendMessage failed:', from, '->', to, res.status, await res.text().catch(() => ''));
    }
    // FIFO: держим не больше 100 сообщений на получателя — старые за пределами лимита удаляем.
    trimMailboxToLimit(to).catch((e) => console.error('trimMailboxToLimit exception:', to, e.message));
  } catch (e) {
    console.error('dbSendMessage exception:', from, '->', to, e.message);
  }
}
export async function trimMailboxToLimit(to, limit = 100) {
  ...
  if (!res.ok) {
    console.error('trimMailboxToLimit load failed:', to, res.status, await res.text().catch(() => ''));
    return;
  }
...
export async function dbMarkMessageRead(messageId) {
  try {
    const res = await sbFetch(`messages?id=eq.${messageId}`, { method: 'PATCH', body: JSON.stringify({ read: true }) });
    if (!res.ok) {
      console.error('dbMarkMessageRead failed:', messageId, res.status, await res.text().catch(() => ''));
    }
  } catch (e) {
    console.error('dbMarkMessageRead exception:', messageId, e.message);
  }
}
export async function dbMarkAllRead(nickname) {
  try {
    const res = await sbFetch(`messages?to_nickname=eq.${encodeURIComponent(nickname)}&read=eq.false`, { method: 'PATCH', body: JSON.stringify({ read: true }) });
    if (!res.ok) {
      console.error('dbMarkAllRead failed:', nickname, res.status, await res.text().catch(() => ''));
    }
  } catch (e) {
    console.error('dbMarkAllRead exception:', nickname, e.message);
  }
}
export async function dbDeleteMessages(ids) {
  ...
  try {
    ...
    const res = await sbFetch(`messages?id=in.(${idStr})`, { method: 'DELETE' });
    if (!res.ok) {
      console.error('dbDeleteMessages failed:', ids.length, 'ids', res.status, await res.text().catch(() => ''));
    }
  } catch (e) {
    console.error('dbDeleteMessages exception:', e.message);
  }
}
