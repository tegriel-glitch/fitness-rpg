...
const MORNING_RITUALS = [
  { id: 'sunrise',   emoji: '🌅', name: 'Приветствие солнца',  desc: '5 минут растяжки после пробуждения',     stat: 'Гибкость',  xpBonus: 0.05, classHint: 'monk' },
  { id: 'water',     emoji: '💧', name: 'Стакан воды',          desc: 'Выпить стакан воды сразу после подъёма', stat: 'ХП',        xpBonus: 0.05, classHint: 'shaman' },
  { id: 'page',      emoji: '📖', name: 'Страница мудрости',    desc: 'Прочитать хотя бы одну страницу',        stat: 'Интеллект', xpBonus: 0.05, classHint: 'archmage' },
  { id: 'breath',    emoji: '🧘', name: 'Три вдоха',            desc: '1 минута осознанного дыхания',            stat: 'Дух',       xpBonus: 0.05, classHint: 'shaman' },
  { id: 'cold',      emoji: '⚡', name: 'Холодный финиш',      desc: '30 секунд холодного душа',               stat: 'Упорство',  xpBonus: 0.05, classHint: 'berserker' },
  { id: 'intention', emoji: '🎯', name: 'Намерение дня',        desc: 'Сформулировать одну цель на день',       stat: 'Воля',      xpBonus: 0.05, classHint: 'pathfinder' },
];
...
const RAID_BOSSES = [
  ...
  },
  {
    ...
    lootTier: 'rare',
    description: '10 дней. Суммарно 30 силовых тренировок (зал или парк) на всех участников.',
    condition: {
      type: 'shared_count',
  ...
  },
  {
    ...
    lootTier: 'epic',
    description: '14 дней. Каждый выбирает роль: Охотник (≥5 тренировок борьбы) или Ветер (≥70 км бега). Все роли должны быть закрыты.',
    condition: {
      type: 'combo_roles',
      label: 'Роли выполнены',
      roles: {
        hunter: { activity: 'wrestling', target: 5, unit: 'тренировок', label: '🐯 Охотник' },
        wind:   { activity: 'running',   target: 70, field: 'distance', unit: 'км', label: '💨 Ветер' },
      },
    },
  },
  {
...
export default function App() {
  ...
  useEffect(() => {
    stateRef.current = {
      ...
      raids, guildLikes, lastLoginDate, loginStreak, morningRitualLog,
    };
  });
  ...
  async function selectNickname(nick) {
    ...
    try {
      ...
      if (row.raids != null)                 setRaids(row.raids);
  ...
  function startRaid(bossId) {
    ...
    setRaids((prev) => ({
      ...prev,
      [bossId]: {
        status: 'gathering', // waiting for all 3 players to join and ready up
        startDate: null,
        startClassId: currentClass ? (currentClass.combo ? currentClass.classA.id : currentClass.id) : null,
        participants: [{ name: characterName, ready: false }],
        contributions: [],
  ...
  function joinRaid(bossId) {
    setRaids((prev) => {
      ...
      if (!raid || raid.status !== 'gathering') return prev;
      const boss = RAID_BOSSES.find(b => b.id === bossId);
      if (raid.participants.length >= (boss.maxPlayers || 3)) return prev;
      if (raid.participants.find(p => p.name === characterName)) return prev;
      return { ...prev, [bossId]: { ...raid, participants: [...raid.participants, { name: characterName, ready: false }] } };
    });
  }
  ...
  function setReady(bossId) {
    setRaids((prev) => {
      ...
      const startDate = allReady ? dateKey(new Date()) : null;
      return { ...prev, [bossId]: { ...raid, participants: updated, status: newStatus, startDate: startDate || raid.startDate } };
    });
    setToast({ text: '✅ Готов к рейду!', key: Date.now() });
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 6, flexWrap: 'wrap' }}>
          ...
          {ritualXpBonus > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '3px 8px', background: '#1a0f2a', border: '1px solid #3a2a5a', borderRadius: 8 }}>
              ...
              <span style={{ fontSize: 10, color: '#c9a8f5', fontWeight: 700 }}>+5% XP</span>
            </div>
          )}
        </div>
      )}
...
function MorningRitualSection({ morningRitualLog, onPerformRitual, ritualXpBonus, currentClassId }) {
  ...
  return (
    <div style={{ marginBottom: 16, background: '#141420', border: '1.5px solid #2a2a3a', borderRadius: 14, overflow: 'hidden' }}>
      <div style={{ padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{ fontSize: 16 }}>🌄</span>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 12, fontWeight: 800, color: '#c9a8f5' }}>Утренний ритуал</div>
          {ritual ? (
            <div style={{ fontSize: 10.5, color: '#4ce0c0', marginTop: 1 }}>
              {ritual.emoji} {ritual.name} · +5% XP · +1 {ritual.stat}
            </div>
          ) : (
            <div style={{ fontSize: 10.5, color: '#5a5a6a', marginTop: 1 }}>
              {isMorning ? 'Выбери ритуал до 12:00 · +5% XP на день' : 'Ритуал не выполнен сегодня'}
            </div>
          )}
        </div>
        {ritualXpBonus > 0 && (
          <div style={{ fontSize: 10, fontWeight: 700, color: '#4ce0c0', background: '#1a3a2a', border: '1px solid #2a5a3a', borderRadius: 6, padding: '2px 8px' }}>
            +5% XP ✓
          </div>
        )}
      </div>
      ...
      {!ritual && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, padding: '0 12px 12px' }}>
          {MORNING_RITUALS.map(r => {
            const isClassHint = r.classHint === currentClassId;
            return (
              <button
                key={r.id}
                onClick={() => onPerformRitual(r.id)}
                disabled={!isMorning}
                style={{
                  display: 'flex', alignItems: 'center', gap: 8,
                  padding: '9px 10px', borderRadius: 10, cursor: isMorning ? 'pointer' : 'not-allowed',
                  background: isClassHint ? '#2a1a3a' : '#1a1a24',
                  border: '1px solid ' + (isClassHint ? '#6a3a9a' : '#2a2a32'),
                  opacity: isMorning ? 1 : 0.5, textAlign: 'left',
                }}
              >
                <span style={{ fontSize: 18, flexShrink: 0 }}>{r.emoji}</span>
                <div>
                  <div style={{ fontSize: 11, fontWeight: 700, color: isClassHint ? '#c9a8f5' : '#c0c0ca', lineHeight: 1.2 }}>{r.name}</div>
                  <div style={{ fontSize: 9.5, color: '#5a5a6a', marginTop: 1 }}>+1 {r.stat}</div>
                </div>
              </button>
            );
          })}
...
function RaidsView({ raids, currentClass, characterName, onStartRaid, onJoinRaid, onSetReady, onAddContribution, onResolve, onAbandon }) {
  return (
    <div style={{ animation: 'fadeUp 0.3s ease' }}>
...
function RaidBossCard({ boss, raid, currentClass, characterName, onStart, onJoin, onReady, onAddContribution, onResolve, onAbandon }) {
    ...
    <div style={{ border: `1.5px solid ${rc.border}`, background: rc.bg, borderRadius: 16, padding: 16, marginBottom: 16 }}>
      ...
      {status === 'gathering' && (
        <div style={{ background: '#1a1a22', border: '1px solid #2a2a32', borderRadius: 12, padding: 12, marginBottom: 12 }}>
          ...
          {isParticipant && (
            <button onClick={onAbandon} style={{ width: '100%', marginTop: 6, padding: '6px', borderRadius: 8, cursor: 'pointer', background: 'none', border: '1px solid #3a2a2a', fontSize: 11, color: '#6a4a4a' }}>
              Выйти из рейда
            </button>
          )}
        </div>
      ...
      {/* Abandon completed raids */}
      {(status === 'victory' || status === 'defeat') && (
        <button onClick={onAbandon} style={{ width: '100%', padding: '8px', borderRadius: 8, cursor: 'pointer', background: 'none', border: '1px solid #2a2a32', fontSize: 11, color: '#5a5a6a', marginTop: 4 }}>
          Закрыть
        </button>
      )}
    </div>
...
function GuildView({ playerName, playerCharacterName, playerLevel, playerClass, playerPhysical, playerMental, playerTitleEntry, activeTitle, earnedTitles, raids, guildLikes, onLike, lockedClassId, chosenPathId, guildMembers, onRefreshGuild, onLogout }) {
  ...
  const activeRaidIds = RAID_BOSSES
    .filter(b => raids[b.id]?.status === 'active' || raids[b.id]?.status === 'gathering')
    .map(b => b.id);
