  ...
  useEffect(() => {
    Object.entries(raids).forEach(([bossId, raid]) => {
      ...
      if (shouldArchive) {
        ...
        classByName.set(characterName, lockedClassId || currentClass?.id || classId);
        (guildMembers || []).forEach((m) => {
          const nm = m.character_name || m.nickname;
          if (nm) classByName.set(nm, approxGuildMemberClassId(m));
        });
        const participantClasses = raid.participants.map(p => ({ name: p.name, classId: classByName.get(p.name) || null }));
        const archiveEntry = {
...
const ArchiveDetailModal = React.memo(function ArchiveDetailModal({ entry, onClose, guildMembers }) {
  ...
  const story = React.useMemo(() => {
    ...
    const classByName = new Map((guildMembers || []).map((m) => [m.name || m.characterName, m.locked_class_id || null]));
    const savedByName = new Map((entry.participantClasses || []).map((p) => [p.name, p.classId || null]));
    ...
    const participants = names.map((name) => ({
      name,
      classId: savedByName.get(name) || classByName.get(name) || null,
    }));
