...
async function dbLoadGuildMembers(excludeNickname) {
  const res = await sbFetch(`players?nickname=neq.${encodeURIComponent(excludeNickname)}&select=nickname,character_name,locked_class_id,chosen_path_id,logs,passive_logs,recovery_logs,raids,active_title,likes,purchased_item_ids,equipped_shop_items,unlocked_skill_levels,books,equipped_avatar_frame`);
  if (!res.ok) return [];
...
const AVATAR_FRAMES = [
  ...
  // === ОБЫЧНЫЕ ===
  { id: 'frame_common_silver', name: 'Серебряный обод',  rarity: 'common',   price: 20,
    style: { border: '3px solid #c0c0c0', boxShadow: '0 0 8px #c0c0c033' } },
  { id: 'frame_common_glow',   name: 'Тихое свечение',   rarity: 'common',   price: 25,
    style: { border: '2px solid #e0e0f0', boxShadow: '0 0 12px #9090d055, 0 0 4px #c0c0f077' } },
  { id: 'frame_common_bronze', name: 'Бронзовая оправа', rarity: 'common',   price: 22,
    style: { border: '3px solid #cd7f32', boxShadow: '0 0 6px #cd7f3233' } },

  // === РЕДКИЕ ===
  { id: 'frame_rare_branches', name: 'Ветви леса',        rarity: 'rare',    price: 80,
    style: { border: '3px solid #5a8a3a', boxShadow: '0 0 10px #5a8a3a55, inset 0 0 8px #2a5a1a33' } },
  { id: 'frame_rare_leaves',   name: 'Венок из листьев', rarity: 'rare',    price: 85,
    style: { border: '3px solid #7ec850', boxShadow: '0 0 14px #7ec85066, 0 0 4px #4a8a2a55' } },
  { id: 'frame_rare_crystals', name: 'Кристальный ореол', rarity: 'rare',   price: 90,
    style: { border: '3px solid #5b9bf0', boxShadow: '0 0 14px #5b9bf077, 0 0 4px #9ad0f555' } },
  { id: 'frame_rare_fire',     name: 'Огненный обод',    rarity: 'rare',    price: 95,
    style: { border: '3px solid #e8633c', boxShadow: '0 0 16px #e8633c88, 0 0 6px #f5a05566' } },
  { id: 'frame_rare_frost',    name: 'Ледяная кромка',   rarity: 'rare',    price: 90,
...
function CosmeticsSection({ purchasedFrameIds, equippedAvatarFrame, onBuyFrame, onEquipFrame, currencyBalance }) {
  const rarityGroups = ['cheap', 'common', 'rare', 'epic', 'legendary', 'mythic'];
  const tierColors = {
