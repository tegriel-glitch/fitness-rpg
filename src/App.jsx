              ...
              <div style={{ ...styles.shopCardIconWrap, background: rarity.color + '22' }}>
                {available || owned
                  ? (item.image
                      ...
                      : <Icon size={40} color={rarity.color} />)
                  : <Lock size={32} color="#5a5a62" />}
              </div>
    ...
    <div style={{ animation: 'fadeUp 0.3s ease' }}>
      ...
      {chestOpen && (
        <div
          ...
          onClick={(e) => { if (e.target === e.currentTarget) setChestOpen(false); }}
        >
          <div style={{ animation: 'fadeUp 0.2s ease', background: '#18181f', border: '1.5px solid #28282f', borderRadius: 16, padding: 14, maxWidth: 480, width: '100%', maxHeight: '86vh', overflowY: 'auto', position: 'relative' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, position: 'sticky', top: -14, background: '#18181f', paddingTop: 2, paddingBottom: 8, zIndex: 2 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <ShoppingBag size={16} color="#f0d272" />
                <span style={{ fontWeight: 800, fontSize: 14, color: '#f0f0f4' }}>Сундук экипировки</span>
              </div>
              <button onClick={() => setChestOpen(false)} style={{ background: '#1c1c22', border: '1px solid #28282f', borderRadius: 8, cursor: 'pointer', padding: 5, display: 'flex' }}>
                <X size={16} color="#9a9aa2" />
              </button>
            </div>
            <EquippedGear equippedShopItems={equippedShopItems} lockedClassId={lockedClassId} onSell={onSell} />
            {/* Рамка аватара */}
...
const EquippedGear = React.memo(function EquippedGear({ equippedShopItems, lockedClassId, onSell }) {
  ...
  function renderSlotCard(slot) {
    ...
    return (
      ...
      >
        <div style={{ ...styles.equippedGearIconWrap, background: rarity ? rarity.color + '22' : '#20202a', overflow: 'hidden' }}>
          {item
            ? (item.image
                ? <img src={item.image} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                : <item.icon size={28} color={rarity.color} />)
            : <Lock size={22} color="#4a4a52" />}
        </div>
        ...
        {item && item.price > 0 && onSell && (
          <button
            ...
            style={{
              ...
              background: 'rgba(10,10,13,0.75)', border: '1px solid #3a3a42', color: '#8a8a92', cursor: 'pointer', padding: 0,
            }}
          >
            <Gem size={9} />
          </button>
