...
async function dbSavePlayer(nickname, snapshot, opts = {}) {
  ...
  // keepalive гарантирует доставку при закрытии вкладки, но ограничен ~64 КБ.
  // Если тело превышает лимит, fetch с keepalive бросит TypeError.
  // Fallback: шлём БЕЗ keepalive — браузер МОЖЕТ оборвать, но хотя бы не молча проглотит.
  const wantKeepalive = !!opts.keepalive;
  const safeForKeepalive = body.length < 60000; // 60 KB с запасом от 64 KB лимита
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error('[FitnessRPG] save FAILED:', res.status, errText, '| body size:', body.length);
    }
  } catch (e) {
    console.error('[FitnessRPG] save exception:', e.message, '| keepalive:', useKeepalive, '| body size:', body.length);
    // Если keepalive превысил квоту (или сеть упала), пробуем без keepalive
    if (useKeepalive) {
      try {
        ...
        if (!res2.ok) console.error('[FitnessRPG] save retry FAILED:', res2.status);
...
function AppInner() {
  ...
  function scheduleSave(overrides = {}) {
    ...
    saveTimerRef.current = setTimeout(async () => {
      ...
      console.log('[save] →', nicknameRef.current, '| logs:', logCount, '| body:', JSON.stringify(snap).length, 'B');
      try {
        const res = await sbFetch(`players?nickname=eq.${encodeURIComponent(nicknameRef.current)}`, {
          method: 'PATCH',
          body: JSON.stringify(snap),
        });
        if (!res.ok) {
          const errText = await res.text().catch(() => '');
          console.error('[save] FAIL', res.status, errText);
          setSaveStatus('error');
        } else {
          console.log('[save] ✓ OK');
          setSaveStatus('ok');
          ...
          try {
            ...
            if (vLogs !== logCount) {
              ...
              console.log('[save] ✓ verified:', vLogs, 'logs in DB');
            }
          } catch(_) {}
        }
      } catch (e) {
