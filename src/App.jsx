...
export default function App() {
  ...
  function startRaid(bossId) {
    ...
    setRaids((prev) => ({
      ...
      [bossId]: {
        ...
        participants: [], // creator joins via joinRaid after starting
        contributions: [],
  ...
  function joinRaid(bossId, role) {
    setRaids((prev) => {
      ...
      // For combo_roles raids, check role isn't taken
      if (boss.condition.type === 'combo_roles' && role) {
        if (raid.participants.find(p => p.role === role)) return prev;
      }
      return { ...prev, [bossId]: { ...raid, participants: [...raid.participants, { name: characterName, ready: false, role: role || null }] } };
    });
  }
  ...
  function addRaidContribution(bossId, participantName, value) {
    ...
    setRaids((prev) => {
      ...
      if (boss.condition.type === 'shared_total') {
        ...
        const total = newContribs.length; // each contribution = 1 training
        if (total >= boss.condition.target) victory = true;
      } else if (boss.condition.type === 'combo_roles') {
        // Each participant must hit their role target
        victory = raid.participants.every(p => {
          ...
          const total = pContribs.reduce((s, c) => s + c.value, 0);
...
function MorningRitualSection({ morningRitualLog, onPerformRitual, ritualXpBonus, currentClassId }) {
  ...
  const [open, setOpen] = useState(!ritual && isMorning);
  const isDone = !!ritual;
  ...
  return (
    <div style={{
      ...
      background: isDone ? '#141420' : '#1a1428',
      border: `1.5px solid ${isDone ? '#2a2a3a' : '#6a3a9a'}`,
      boxShadow: isDone ? 'none' : '0 0 18px #7a3aaa55, 0 0 6px #c9a8f533',
      ...
      >
        <span style={{ fontSize: 18 }}>{isDone ? ritual.emoji : '🌄'}</span>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 12, fontWeight: 800, color: isDone ? '#c9a8f5' : '#d4b8ff' }}>
            Утренний ритуал
          </div>
          <div style={{ fontSize: 10.5, marginTop: 1, color: isDone ? '#4ce0c0' : isMorning ? '#9a6acd' : '#4a4a5a' }}>
            {isDone
              ? `${ritual.name} · +5% XP · +1 ${ritual.stat}`
              : isMorning ? 'Нажми чтобы выбрать ритуал · +5% XP на день' : 'Ритуал не выполнен сегодня'}
          </div>
        </div>
        {isDone
          ? <div style={{ fontSize: 10, fontWeight: 700, color: '#4ce0c0', background: '#1a3a2a', border: '1px solid #2a5a3a', borderRadius: 6, padding: '2px 8px' }}>✓ Готово</div>
          : isMorning && <div style={{ fontSize: 18, color: '#c9a8f5', animation: open ? 'none' : 'pulse 2s infinite' }}>✨</div>
        }
      ...
      {open && !isDone && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, padding: '0 12px 12px' }}>
          {MORNING_RITUALS.map(r => (
            <button
              key={r.id}
              onClick={() => isMorning && handlePick(r.id)}
              disabled={!isMorning}
              style={{
                display: 'flex', alignItems: 'center', gap: 8,
                padding: '9px 10px', borderRadius: 10,
                cursor: isMorning ? 'pointer' : 'not-allowed',
            ...
            >
              ...
              <div>
                <div style={{ fontSize: 11, fontWeight: 700, color: '#d0b8f5', lineHeight: 1.2 }}>{r.name}</div>
                <div style={{ fontSize: 9.5, color: '#6a5a7a', marginTop: 1 }}>+1 {r.stat}</div>
              </div>
            </button>
          ))}
        </div>
    ...
    <div style={{ border: `1.5px solid ${rc.border}`, background: rc.bg, borderRadius: 16, padding: 16, marginBottom: 16, position: 'relative', overflow: 'hidden' }}>
      ...
      {boss.creature && (
        <div style={{
          ...
          fontSize: 88, opacity: 0.08, pointerEvents: 'none',
