          ...
          {ritualXpBonus > 0 && (() => {
            ...
            return (
              <div style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '3px 8px', background: '#1a0f2a', border: '1px solid #3a2a5a', borderRadius: 8 }}>
                {todayRitual?.icon ? (
                  <img src={todayRitual.icon} alt={todayRitual.name} style={{ width: 14, height: 14, borderRadius: 4, objectFit: 'cover', flexShrink: 0 }} />
                ) : (
    ...
    <div style={{ display: 'flex', gap: 3, alignItems: 'center', marginTop: 2, flexWrap: 'wrap' }}>
      {Array.from({ length: count }).map((_, idx) => (
        <img
          ...
          src={src}
          alt={alt}
          style={{ width: 14, height: 14, borderRadius: 4, animation: 'popIn 0.3s ease' }}
        />
      ))}
    </div>
  );
