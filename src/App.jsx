...
import {
  ...
  RAID_LOOT_SETS_BY_CLASS, REPEATABLE_SET_BOSS_IDS, RARITY_ACCESS_THRESHOLD,
} from './game/raidBosses';
import { composeBattleChronicle } from './game/battleChronicles';
...
// Приблизительное определение класса тиммейта в гильдии ДО того, как он его залочил
// (level < 10). До этого момента `locked_class_id` в базе всегда null, а живой отряд гильдии
// (GuildView) для отображения бейджа считает "динамический" класс из статов текущих логов —
// но эта логика раньше нигде не переиспользовалась вне GuildView, поэтому хроники битвы
// (composeBattleChronicle) для рейдов, пройденных до 10 уровня хотя бы одним из участников,
// не находили classId вообще ни у кого и молча падали в нейтральный generic-текст.
//
// Это НАМЕРЕННО упрощённая копия расчёта из GuildView.memberFromRow — без учёта множителя
// стрика (влияет только на магнитуду опыта, а не на то, какая статгруппа лидирует, в
// подавляющем большинстве случаев). Используется ИСКЛЮЧИТЕЛЬНО для выбора текста хроники —
// никогда не для начисления опыта/статов/наград, поэтому расхождение с точным движком гильдии
// здесь не создаёт риска "тихо сломать лидерборд", в отличие от settings, где такая
// рассинхронизация была бы критичной.
function approxGuildMemberClassId(row) {
  if (!row) return null;
  if (row.locked_class_id) return row.locked_class_id;
  if (!row.logs || !row.logs.length) return null;
  const totals = {};
  ALL_STATS.forEach((s) => { totals[s] = 0; });
  row.logs.forEach((l) => {
    const def = ACTIVITY_TYPES[l.activity];
    if (!def || !def.stats || !def.stats.length) return;
    let base = 0;
    if (l.activity === 'running') {
      const d = Number(l.distance) || 0;
      if (d > 0) base = Math.round(Math.min(14, 6 + d * 0.65));
    } else if (l.activity === 'reading') {
      const p = Number(l.pages) || 0;
      if (p >= 100) base = 8; else if (p >= 50) base = 6; else if (p >= 20) base = 2;
    } else if (l.activity === 'calories') {
      base = 0;
    } else if (INTENSITY_ACTIVITIES.has(l.activity)) {
      base = (INTENSITY_LEVELS[l.intensity] || INTENSITY_LEVELS.medium)?.xp || 10;
    } else if (l.activity === 'nutrition') {
      base = 4;
    } else if (l.activity === 'sleep') {
      base = 5;
    } else {
      base = 4;
    }
    if (base <= 0) return;
    def.stats.forEach((s) => { if (totals[s] !== undefined) totals[s] += base; });
  });
  const level = (row.current_level && row.current_level > 0) ? row.current_level : 1;
  const resolved = resolveCharacterClass(totals, level, row.combo_class_id || null);
  if (!resolved) return null;
  return resolved.combo ? (resolved.classA?.id || null) : (resolved.id || null);
}

// Нормализация имени персонажа для сопоставления по строке (участники рейда/архива vs
// ростер гильдии) — на случай лишних пробелов по краям или регистра, из-за которых участник
// мог не находиться в classByName и молча уходить в null, даже когда его класс на самом деле
// уже залочен и виден во вкладке «Гильдия».
function normName(s) {
  return (s || '').trim().toLowerCase().replace(/\s+/g, ' ');
}

// Единая функция подсчёта прогресса shared_count/shared_total — раньше эта логика была
// продублирована в 4 разных местах (авто-вклад, victory-проверка, прогресс-бар карточки,
// экран участников), и для "дневных" боссов (Скорпион/Кракен) везде был один и тот же баг:
// считался сырой массив contributions.length, а не число УНИКАЛЬНЫХ дней на игрока — несколько
// логов активности за один день ошибочно засчитывались как несколько дней сразу.
function countSharedProgress(contributions, cond) {
  if (cond.type === 'shared_total') {
    return contributions.reduce((s, c) => s + c.value, 0);
  }
  if (cond.type === 'shared_count') {
    if (cond.dedupeByDate) {
      return new Set(contributions.map((c) => `${c.participantName}|${c.date}`)).size;
    }
...
// Получен ли трофей за первую победу над боссом — с учётом обратной совместимости: игроки,
// победившие босса ДО того, как трофей и сет разделили на два отдельных механизма, не имеют
// записи в raidTrophiesReceived, но у них уже есть архивная запись победы — это то же самое
// по смыслу, и без этой проверки таким игрокам казалось бы, что трофей ещё не выдан, а сет
// никогда не начинал бы собираться.
function hasReceivedTrophy(bossId, raidTrophiesReceived, raidArchive) {
  if ((raidTrophiesReceived || []).includes(bossId)) return true;
  return (raidArchive || []).some((e) => e.bossId === bossId && e.result === 'victory');
}
...
function AppInner() {
  ...
  // raidSetProgress: { [bossId]: [полученные itemId сета] } — см. game/saveableFields.js
  const [raidSetProgress, setRaidSetProgress] = useState({});
  // raidTrophiesReceived: [bossId, ...] — одноразовый трофей за первую победу, отдельно от сета
  const [raidTrophiesReceived, setRaidTrophiesReceived] = useState([]);
  ...
  const fieldSetters = useMemo(() => ({
    ...
    raidSetProgress: setRaidSetProgress,
    raidTrophiesReceived: setRaidTrophiesReceived,
  ...
  const refreshRaids = useCallback(async () => {
    ...
    try {
      ...
      // `raids` feeds the autosave effect's dependency list — replacing it with a brand-new
      // object every 12s (even when nothing actually changed) was triggering a spurious save
      // roughly every 12-13 seconds purely from this background poll, on top of real user
      // actions. Only commit the new object if its content actually differs.
      setRaids((prev) => (JSON.stringify(prev) === JSON.stringify(fresh) ? prev : fresh));
    } catch (_) {}
  }, [selectedNickname]);
  ...
  useEffect(() => {
    ...
    morningRitualLog, shopShields, lastShopShieldUse, lastRaidShieldUse, raidArchive, unomieDebuff, raidSetProgress, raidTrophiesReceived,
  ...
  const healthState = useMemo(() => {
    ...
    combined.forEach((entry) => {
      ...
      // Hysteresis: dropping below 30% locks XP gain for that bar; it stays locked until the
      // bar recovers to 50%. (Previously this checked `<= 0` instead of `< 30` — meaning XP
      // only locked at literal zero health, not "below 30%" as designed — that mismatch is
      // exactly what let a player keep earning XP, and even level up multiple times in one
      // sitting, while sitting near-zero HP that never happened to hit the literal 0 mark.)
      if (physical < 30) physicalXpLocked = true;
      else if (physical >= 50) physicalXpLocked = false;
      if (mental < 30) mentalXpLocked = true;
  ...
  const statTotals = useMemo(() => {
    ...
    // Deduplicate by bossId (same boss can only grant its trophy bonus once — this is precisely
    // the "Трофей" mechanic: first victory over ANY boss grants a one-time stat bonus from the
    // legacy single-item data, repeatable or not. For repeatable rare/epic bosses this now runs
    // alongside the per-collected-set-item bonus below, not instead of it.
    ...
    // Repeatable rare/epic set loot — bonus is per COLLECTED ITEM (raidSetProgress), not a
    // single flat bonus per boss. This was the missing piece that made every set item after
    // the first purely cosmetic: itemIds were granted and toasted, but nothing ever summed
    // their `.stats` into the numbers that actually gate levels/classes/avatars.
    Object.entries(raidSetProgress || {}).forEach(([bossId, claimedItemIds]) => {
      const byClass = RAID_LOOT_SETS_BY_CLASS[bossId];
      if (!byClass) return;
      const allItemsForBoss = Object.values(byClass).flat();
      (claimedItemIds || []).forEach((itemId) => {
        const item = allItemsForBoss.find((it) => it.itemId === itemId);
  ...
  // Combined roster (guild + self) for battle-chronicle class lookups — memoized so it keeps a
  // stable reference across renders. This used to be rebuilt inline as a new array literal on
  // every single render of AppInner (many times a minute from polling/re-renders alone), which
  // fed straight into ArchiveDetailModal's useMemo deps and made it recompute the whole battle
  // chronicle far more often than "once per time you actually open a chronicle".
  //
  // Also resolves classId for teammates who haven't locked a class yet (level < 10) via
  // approxGuildMemberClassId — before this fix, a raid fought while any participant was still
  // pre-lock would find `locked_class_id: null` for them and silently fall back to the
  // generic, no-flavor chronicle text for the WHOLE raid, not just that one player.
  const guildRosterWithSelf = React.useMemo(() => (
    [
      ...guildMembers.map(m => ({
        name: m.character_name || m.nickname,
        locked_class_id: approxGuildMemberClassId(m),
        characterName: m.character_name,
      })),
      { name: characterName, locked_class_id: lockedClassId || currentClass?.id || null, characterName },
    ]
  ), [guildMembers, characterName, lockedClassId, currentClass]);
  ...
  function undoLog(logId) {
    ...
    if (pending && pending.logId === logId) {
      ...
      // Roll back "Стена подвигов" (personalRecords/recordsBroken are monotonic and
      // don't auto-revert with `logs`, unlike achievements which recompute from logs directly).
      if (pending.recordsBefore !== undefined) setPersonalRecords(pending.recordsBefore);
      if (pending.recordsBrokenBefore !== undefined) setRecordsBroken(pending.recordsBrokenBefore);
  ...
  function submitLog() {
    ...
    // Snapshot "Стена подвигов" state BEFORE this log lands, so undoLog can restore it exactly —
    // personalRecords/recordsBroken are monotonic high-water marks and don't auto-revert with `logs`.
    const recordsBeforeSnapshot = personalRecords;
    const recordsBrokenBeforeSnapshot = recordsBroken;
    setLogs([...logs, newLog]);
    ...
    pendingLogSideEffects.current = {
      logId, bestiaryTimer, roadTimer, raidContribAfter: raidContribTimestamp,
      recordsBefore: recordsBeforeSnapshot, recordsBrokenBefore: recordsBrokenBeforeSnapshot,
    };
    ...
    RAID_BOSSES.forEach(boss => {
      ...
      // shared_count — тренировки (силовые/борьба) ИЛИ "дни" (питание/сон, dedupeByDate)
      else if (cond.type === 'shared_count') {
        const acts = cond.activities || (cond.activity ? [cond.activity] : []);
        const alreadyContributedToday = cond.dedupeByDate && raid.contributions.some(
          (c) => c.participantName === characterName && c.date === raidToday
        );
        if (alreadyContributedToday) {
          // Уже засчитан сегодняшний день этому игроку — повторный лог того же дня не должен
          // давать ещё один "день" в зачёт (см. dedupeByDate в game/raidBosses.js).
        } else if (acts.includes(newLog.activity)) {
          addRaidContribution(boss.id, characterName, 1);
        } else if (!cond.dedupeByDate && newLog.activity === 'walking' && (Number(newLog.steps) || 0) >= 8000) {
          // Бонусный вклад за 8000+ шагов — только для "тренировочных" боссов (Голем/Левиафан).
          // Для "дневных" (Скорпион/Кракен) шаги тематически не заменяют питание или сон.
          addRaidContribution(boss.id, characterName, 1);
        }
      }
  ...
  function startRaid(bossId) {
    ...
    // Доступ к legendary/mythic — по сумме двух статов залоченного класса (та же метрика,
    // что у порогов открытия аватарок), лично у каждого игрока.
    const accessThreshold = RARITY_ACCESS_THRESHOLD[boss.rarity];
    if (accessThreshold) {
      ...
      const classStatTotal = lockedClassDef
        ...
        : 0;
      if (classStatTotal < accessThreshold) {
        setToast({ text: `🔒 Нужно ${accessThreshold} характеристик класса (сейчас ${classStatTotal})`, key: Date.now() });
        setTimeout(() => setToast(null), 3000); return;
      }
    }

    // Повторяемые сет-боссы (rare/epic) — если сет по своему классу уже собран полностью,
    // рейд для этого игрока личный закрыт (не мешает остальным, у кого сет ещё не собран).
    if (REPEATABLE_SET_BOSS_IDS.includes(bossId)) {
      const classId = currentClass ? (currentClass.combo ? currentClass.classA.id : currentClass.id) : 'pathfinder';
      const setItems = RAID_LOOT_SETS_BY_CLASS[bossId]?.[classId] || [];
      const claimed = raidSetProgress[bossId] || [];
      if (setItems.length > 0 && claimed.length >= setItems.length) {
        setToast({ text: '✅ Ты уже собрал весь сет с этого босса', key: Date.now() });
        setTimeout(() => setToast(null), 3000); return;
      }
    }
  ...
  function addRaidContribution(bossId, participantName, value, override = false, subKey = null) {
    ...
    if (boss.condition.type === 'shared_total') {
      const total = countSharedProgress(newContribs, boss.condition);
  ...
  function resolveRaidDeadline(bossId) {
    ...
    let victory = false;
    if (boss.condition.type === 'shared_total') {
      victory = countSharedProgress(raid.contributions, boss.condition) >= boss.condition.target;
    } else if (boss.condition.type === 'shared_count') {
      victory = countSharedProgress(raid.contributions, boss.condition) >= boss.condition.target;
    } else if (boss.condition.type === 'combo_roles') {
      victory = raid.participants.every(p => {
  ...
  // Cancel a raid (stalled gathering/active raid), or manually trigger shared-record cleanup
  // for a completed one. Archiving itself no longer happens here — see the automatic archiving
  // effect above, which fires the moment a player's own client observes a completed raid,
  // independent of any button click. This function now only ever needs to: (a) mark this
  // player as done with a completed raid (in the rare case the automatic effect hasn't run yet
  // for some reason — this branch is a safety net, not the primary path any more), and (b)
  // actually cancel a still-active/gathering raid, which has no archive step at all.
  function closeRaid(bossId) {
    const raid = raids[bossId];
    const boss = RAID_BOSSES.find(b => b.id === bossId);
    const isCompleted = raid && boss && (raid.status === 'victory' || raid.status === 'defeat');

    if (isCompleted) {
      const closedBy = [...new Set([...(raid.closedBy || []), characterName])];
      const everyoneClosed = raid.participants.every(p => closedBy.includes(p.name));
      if (everyoneClosed) {
        // Last participant to close — safe to remove the shared record for good.
        setRaids((prev) => { const u = { ...prev }; delete u[bossId]; return u; });
        dbDeleteRaid(bossId);
      } else {
        // Others haven't archived it yet on their own clients — keep the shared record
        // alive, just mark that this player is done with it (their card will show it as
        // already closed instead of re-prompting them).
        const updatedRaid = { ...raid, closedBy };
        setRaids((prev) => ({ ...prev, [bossId]: updatedRaid }));
        persistRaid(bossId, updatedRaid);
      }
    } else {
      // Cancelling a stalled gathering/active raid — no completion state to preserve for
      // anyone else, safe to remove immediately as before.
      setRaids((prev) => { const u = { ...prev }; delete u[bossId]; return u; });
      dbDeleteRaid(bossId);
    }
  }

  // Archive a completed raid for THIS player automatically, the moment their own client
  // observes it — independent of the manual "Закрыть" button, same pattern as loot-granting
  // above. This closes a real gap that bit two players: loot/XP/title were already granted by
  // the effect above (automatic, no click needed), but the archive entry only ever got created
  // inside closeRaid(), which required a manual click — so a player could receive full rewards
  // and still end up with the raid missing from their own "Хроники битвы". Now archiving fires
  // on the same automatic trigger as rewards; the manual button becomes a pure cleanup action
  // (removing the shared record once everyone's client has archived it), not a prerequisite.
  useEffect(() => {
    Object.entries(raids).forEach(([bossId, raid]) => {
      if (!raid || (raid.status !== 'victory' && raid.status !== 'defeat')) return;
      if (!raid.participants?.some(p => p.name === characterName)) return;
      if (raid.closedBy?.includes(characterName)) return;
      const boss = RAID_BOSSES.find(b => b.id === bossId);
      if (!boss) return;

      const isRepeatable = REPEATABLE_SET_BOSS_IDS.includes(bossId);
      const trophyAlreadyReceived = hasReceivedTrophy(bossId, raidTrophiesReceived, raidArchive);
      // Хроника/архивная запись на ПОБЕДУ создаётся только один раз — на самую первую победу
      // над этим боссом (тот самый момент трофея). Повторные прохождения (сбор сета) не плодят
      // новые записи в архиве — только тост о полученном предмете (см. эффект наград ниже).
      // Поражения архивируются всегда — они не участвуют в "трофей/сет" механике вообще.
      const shouldArchive = raid.status === 'defeat' || !isRepeatable || !trophyAlreadyReceived;

      if (shouldArchive) {
        const classId = raid.startClassId || effectiveClassId || 'pathfinder';
        const lootData = RAID_LOOT_BY_CLASS[bossId]?.[classId];
        // Для повторяемых боссов archiving случается ровно в момент трофея (первая победа) —
        // поэтому тут всегда трофей, а не предмет сета (тот появится только со второй победы).
        const lootName = lootData?.name || null;

        const classByName = new Map();
        classByName.set(normName(characterName), lockedClassId || currentClass?.id || classId);
        (guildMembers || []).forEach((m) => {
          const nm = m.character_name || m.nickname;
          if (nm) classByName.set(normName(nm), approxGuildMemberClassId(m));
        });
        const participantClasses = raid.participants.map(p => ({ name: p.name, classId: classByName.get(normName(p.name)) || null }));
        const archiveEntry = {
          bossId, bossName: boss.name, bossCreature: boss.creature, bossImage: boss.image, bossRarity: boss.rarity,
          result: raid.status, participants: raid.participants.map(p => p.name), participantClasses,
          date: dateKey(new Date()), lootName: raid.status === 'victory' ? lootName : null,
          story: null,
        };
      ...
      if (raid.status === 'defeat' && !raid.defeatPenaltyApplied) {
        // Rare/epic (повторяемые сет-боссы) — 5 дней блока вместо стандартных 3, как решили
        // отдельно от базового "Уныния". Legendary/mythic (вне этой системы) — старые 3 дня.
        const blockDays = isRepeatable ? 5 : 3;
        const raidDays = boss.durationDays || 3;
        const blockExpires = new Date(); blockExpires.setDate(blockExpires.getDate() + blockDays);
  ...
  }, [raids, characterName, effectiveClassId, lockedClassId, currentClass, guildMembers, raidSetProgress, raidTrophiesReceived]);
  ...
  useEffect(() => {
    Object.entries(raids).forEach(([bossId, raid]) => {
      ...
      if (isRepeatable) {
        const trophyAlreadyReceived = hasReceivedTrophy(bossId, raidTrophiesReceived, raidArchive);
        if (!trophyAlreadyReceived) {
          // Первая победа над этим боссом — Трофей (старый одноразовый предмет), сет ещё не
          // начинает собираться. Сбор сета стартует со ВТОРОЙ победы.
          const trophyData = RAID_LOOT_BY_CLASS[bossId]?.[classId];
          setRaidTrophiesReceived((prev) => {
            const next = prev.includes(bossId) ? prev : [...prev, bossId];
            setTimeout(() => scheduleSave(), 100);
            return next;
          });
          if (trophyData?.itemId) {
            setPurchasedItemIds((cur) => {
              const updated = cur.includes(trophyData.itemId) ? cur : [...cur, trophyData.itemId];
          ...
          const trophyMsg = trophyData?.name ? `Получен трофей: ${trophyData.name}` : 'Победа засчитана';
          setToast({ text: '🏆 ' + trophyMsg + shieldMsg, key: Date.now() });
          setTimeout(() => setToast(null), 4500);
          if (selectedNickname) dbSendMessage('__system__', selectedNickname, `Победа в рейде! ${trophyMsg}. Со следующей победы начнёт собираться сет лута с этого босса.`, 'Внутренний голос', 'raid');
        } else {
          // Трофей уже получен раньше — начиная со второй победы выдаём предметы сета.
          const setItems = RAID_LOOT_SETS_BY_CLASS[bossId]?.[classId] || [];
          const claimed = raidSetProgress[bossId] || [];
          const nextItem = setItems.find((it) => !claimed.includes(it.itemId));
          if (nextItem) {
            const updatedClaimed = [...claimed, nextItem.itemId];
            const setNowComplete = updatedClaimed.length >= setItems.length;
            setRaidSetProgress((prev) => {
              const next = { ...prev, [bossId]: updatedClaimed };
              setTimeout(() => scheduleSave(), 100);
              return next;
            });
            setPurchasedItemIds((cur) => {
              const updated = cur.includes(nextItem.itemId) ? cur : [...cur, nextItem.itemId];
              setTimeout(() => scheduleSave(), 150);
              return updated;
            });
            setRaidShields(prev => {
            ...
            const setMsg = setNowComplete ? ' — сет собран полностью! 🎉' : ` (${updatedClaimed.length}/${setItems.length})`;
            const shieldMsg = raidShields >= MAX_RAID_SHIELDS ? '' : ' + 🛡️ Рейдовый щит';
            setToast({ text: '🏆 Победа! Получено: ' + nextItem.name + setMsg + shieldMsg, key: Date.now() });
            setTimeout(() => setToast(null), 4500);
            if (selectedNickname) dbSendMessage('__system__', selectedNickname, `Победа в рейде! Получен: ${nextItem.name}${setNowComplete ? ' — сет собран полностью!' : ''}`, 'Внутренний голос', 'raid');
          } else {
            // Защитный случай: сет уже был собран раньше (гейт в startRaid должен был не
            // пустить в повторный рейд, но на всякий случай не выдаём ничего лишнего и не падаем).
            setRaidShields(prev => {
              const next = Math.min(prev + 1, MAX_RAID_SHIELDS);
              setTimeout(() => scheduleSave(), 200);
              return next;
            });
            setToast({ text: '🏆 Победа! Сет уже был собран ранее.', key: Date.now() });
            setTimeout(() => setToast(null), 3000);
          }
        }
      } else {
  ...
  }, [raids, characterName, effectiveClassId, raidSetProgress, raidTrophiesReceived, raidArchive]);
      ...
      <div style={styles.content}>
        ...
        {tab === 'raids' && (
          <RaidsView
            ...
            unomieDebuff={unomieDebuff}
            guildMembers={guildRosterWithSelf}
            raidSetProgress={raidSetProgress}
            raidTrophiesReceived={raidTrophiesReceived}
          />
        )}
        {tab === 'guild' && (
          <GuildView
            ...
            myActiveBackground={activeBackground}
...
const Header = React.memo(function Header({ levelState, statTotals, unlockedCount, totalAchievements, achievements, mythicAchievements, pentaResults, comboCounts, balanceCounts, secretAchievements, receivedLikes, activeShield, ritualXpBonus, morningRitualLog, nudges, onDismissNudges, name, onNameChange, healthState, currentClass, currencyBalance, activeTitle, earnedTitles, setActiveTitle, titleCooldownDays, lockedClassId, chosenPathId, classChoiceMode, comboClassId, comboPathId, unlockedComboSkillLevels, specPathId, unlockedSpecSkillLevels, logs, streaksByActivity, equippedShopItems, equippedAvatarFrame, avatarEmoji, onSelectAvatarEmoji, activeChallengeBuff, challengeFailDebuff, unomieDebuff, raidShields, onUseRaidShield, unreadMailCount, onOpenMail, personalRecords, onOpenRecordsWall, bestiary, activeBackground, setTab }) {
  ...
  return (
    <div style={styles.header}>
          ...
          {ritualXpBonus > 0 && (() => {
            ...
            return (
              <div style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '3px 8px', background: '#1a0f2a', border: '1px solid #3a2a5a', borderRadius: 8 }}>
                <div style={{ position: 'relative', width: 15, height: 15, borderRadius: 4, flexShrink: 0, overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <span style={{ fontSize: 11 }}>{todayRitual ? todayRitual.emoji : '🌄'}</span>
                  {todayRitual?.icon && (
                    <img src={todayRitual.icon} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }}
      ...
      <div style={styles.healthBarsMergedWrap}>
        <HealthBar value={healthState.physical} gradient="linear-gradient(90deg, #4a0d10 0%, #c62a2a 35%, #e8504a 50%, #c62a2a 65%, #4a0d10 100%)" />
        <HealthBar value={healthState.mental} gradient="linear-gradient(90deg, #0a0a5c 0%, #1414a8 35%, #2828d0 50%, #1414a8 65%, #0a0a5c 100%)" />
      </div>
      {(healthState.fatigueDebuffCount > 0 || healthState.stressDebuffCount > 0 || healthState.poisonCount > 0) && (
        <div style={styles.healthDebuffsWrap}>
          {healthState.fatigueDebuffCount > 0 && (
            <DebuffBadge icon={Zap} image={PASSIVE_TYPES.sleep_debt?.bgImage} label="Усталость" count={healthState.fatigueDebuffCount} />
          )}
          {healthState.stressDebuffCount > 0 && (
            <DebuffBadge icon={Zap} image={PASSIVE_TYPES.stress?.bgImage} label="Стресс" count={healthState.stressDebuffCount} />
          )}
...
// База для рамок лута с боссов по редкости — единый стандарт для rare/epic/legendary/mythic,
// используется везде, где показывается лут с рейда (боевой лог, экипировка, инвентарь,
// "Рейдовые сеты"). Новые иконки без собственной рамки (как присланные вручную по классам)
// получают её отсюда, а не полагаются на художника.
export const LOOT_FRAME_BY_RARITY = {
  rare:      { border: '#5b9bf0', bg: '#162236', glow: false },
  epic:      { border: '#a35bf0', bg: '#231832', glow: false },
  legendary: { border: '#f0c14b', bg: '#332710', glow: true },
  mythic:    { border: '#f5a8ff', bg: '#2a1232', glow: true },
};

const LootIcon = React.memo(function LootIcon({ item, rarity, size = 40, borderRadius, locked = false, fallbackIcon: FallbackIcon }) {
  const frame = LOOT_FRAME_BY_RARITY[rarity || item?.rarity] || LOOT_FRAME_BY_RARITY.rare;
  const br = borderRadius ?? Math.round(size * 0.22);
  return (
    <div style={{
      position: 'relative', width: size, height: size, borderRadius: br, overflow: 'hidden', flexShrink: 0,
      background: frame.bg,
      border: `${frame.glow ? 2 : 1.5}px solid ${frame.border}`,
      boxShadow: frame.glow && !locked ? `0 0 ${Math.round(size * 0.3)}px ${frame.border}55` : 'none',
      ...
      {!item?.image && FallbackIcon && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <FallbackIcon size={Math.round(size * 0.5)} color={frame.border} />
        </div>
      )}
      {locked && (
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(10,10,13,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: Math.round(size * 0.35) }}>
          🔒
        </div>
      )}
    </div>
...
const DebuffBadge = React.memo(function DebuffBadge({ icon: Icon, image, label, count }) {
  return (
    <div style={{ position: 'relative', width: 34, height: 34, borderRadius: 9, flexShrink: 0, overflow: 'hidden', background: '#2a1220', border: '1px solid #5a2a3a' }} title={`${label} ×${count}`}>
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Icon size={16} color="#ff5c7a" />
      </div>
      {image && (
      ...
      <div style={{
        position: 'absolute', bottom: -2, right: -2, minWidth: 15, height: 15, borderRadius: 8,
        background: '#ff5c7a', border: '1.5px solid #1c1c22', display: 'flex', alignItems: 'center', justifyContent: 'center',
        fontSize: 9, fontWeight: 800, color: '#1c1c22', padding: '0 3px',
      }}>
        {count}
      </div>
    </div>
...
const HealthBar = React.memo(function HealthBar({ value, gradient }) {
  return (
    <div style={styles.healthBarTrack}>
      <div style={{
        ...styles.healthBarFill, width: `${value}%`, background: gradient,
        boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.25), inset 0 -1px 0 rgba(0,0,0,0.3)',
      }} />
      <span style={styles.healthBarValueInside}>{value}%</span>
    </div>
  );
});
      ...
      {open && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 7, padding: '0 12px 12px' }}>
          {MORNING_RITUALS.map(r => (
            ...
            >
              <div style={{
                position: 'relative', width: 34, height: 34, borderRadius: 9, flexShrink: 0, marginTop: 1,
                background: '#150e20', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden',
              }}>
                <span style={{ fontSize: 18 }}>{r.emoji}</span>
                {r.icon && (
                  <img src={r.icon} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }}
                    onError={(e) => { e.currentTarget.style.display = 'none'; }} />
                )}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
    ...
    return (
      <div style={{ background: '#1c1c22', border: `1.5px solid ${catColor}33`, borderRadius: 12, padding: '10px 14px', marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
          <div style={{ width: 26, height: 26, borderRadius: 7, background: catColor + '22', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden', position: 'relative' }}>
            <CatIcon size={13} color={catColor} />
            {def.icon && (
              <img src={def.icon} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }}
                onError={(e) => { e.currentTarget.style.display = 'none'; }} />
            )}
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
          ...
          return (
            <div key={key} style={{ ...styles.activityCard, gridColumn: isOpen ? '1 / -1' : 'auto' }}>
              <div
                ...
                style={{
                  cursor: 'pointer', position: 'relative', aspectRatio: '16 / 9', width: '100%',
                }}
              >
                {def.bgImage ? (
                ...
                {streak > 0 && (
                  <div style={styles.streakBadge}>
                    <Flame size={12} color="#e0a868" /> {streak}
                  </div>
                )}
      ...
      {Object.entries(PASSIVE_TYPES).filter(([key]) => key !== 'stagnation').map(([key, def]) => {
        ...
        return (
          <button
            ...
            style={{
              ...styles.passiveCard,
              borderColor: active ? '#8a5cf6' : '#28282f',
            }}
          >
            {def.bgImage ? (
              <img src={def.bgImage} alt="" style={styles.passiveBgImage}
                onError={(e) => { e.currentTarget.style.display = 'none'; }} />
            ) : (
              <div style={{ ...styles.passiveIconFallback, background: '#1c1c22' }}>
                <Icon size={26} color="#6a6a72" />
              </div>
            )}
            <div style={{
              ...styles.passiveOverlay,
              background: active
                ? 'linear-gradient(180deg, rgba(106,79,156,0.05) 0%, rgba(20,14,32,0.35) 45%, rgba(20,14,32,0.82) 100%)'
                : 'linear-gradient(180deg, rgba(10,10,13,0.1) 0%, rgba(10,10,13,0.45) 45%, rgba(10,10,13,0.88) 100%)',
            }}>
              <span style={{ ...styles.passiveLabel, color: active ? '#dcc8f5' : '#c8c8ce' }}>{def.label}</span>
            </div>
          </button>
        );
      })}
...
const ArchiveDetailModal = React.memo(function ArchiveDetailModal({ entry, onClose, guildMembers }) {
  const rc = RAID_RARITY_COLORS[entry.bossRarity] || RAID_RARITY_COLORS.rare;

  const story = React.useMemo(() => {
    // participantClasses is only present on entries archived after this feature shipped —
    // but even when present, individual entries can have classId: null baked in forever if the
    // raid was closed back when that participant's class wasn't resolvable yet (before locking,
    // before the dynamic-class fallback existed, etc). The bug this fixes: we used to trust the
    // saved array wholesale the moment it existed with length > 0, even if every classId inside
    // was null — so a raid closed once with unresolved classes stayed broken forever, even
    // after the player went on to lock a real class later. Now each participant is resolved
    // per-name, falling back to the live guild roster whenever the saved classId is missing.
    const classByName = new Map((guildMembers || []).map((m) => [normName(m.name || m.characterName), m.locked_class_id || null]));
    const savedByName = new Map((entry.participantClasses || []).map((p) => [normName(p.name), p.classId || null]));
    const names = entry.participants && entry.participants.length > 0
      ? entry.participants
      : (entry.participantClasses || []).map((p) => p.name);
    const participants = names.map((name) => ({
      name,
      classId: savedByName.get(normName(name)) || classByName.get(normName(name)) || null,
    }));
    return composeBattleChronicle({ bossId: entry.bossId, participants, result: entry.result });
  }, [entry, guildMembers]);
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.88)', zIndex: 80, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}
      ...
      <div style={{ background: '#1c1c22', border: `1.5px solid ${rc.color}44`, borderRadius: 16, padding: 20, width: '100%', maxWidth: 400, maxHeight: '85vh', overflowY: 'auto' }}
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 4 }}>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#5a5a62', fontSize: 20, lineHeight: 1 }}>✕</button>
        </div>

        {/* Крупная картинка босса */}
        {(() => {
          ...
          return (
            <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 14 }}>
              {img
                ? <img src={img} alt="" style={{ width: 160, height: 160, borderRadius: 20, objectFit: 'cover', border: `1.5px solid ${rc.color}55` }} />
                : <span style={{ fontSize: 100 }}>{entry.bossCreature}</span>}
            </div>
          );
        })()}

        {/* Результат + название + дата */}
        <div style={{ textAlign: 'center', marginBottom: 14 }}>
          <div style={{ fontSize: 15, fontWeight: 800, color: entry.result === 'victory' ? '#7de87d' : '#e87d7d', marginBottom: 2 }}>
            {entry.result === 'victory' ? '🏆 Победа' : '💀 Поражение'}
          </div>
          <div style={{ fontSize: 12.5, color: rc.color, fontWeight: 700 }}>{entry.bossName}</div>
          <div style={{ fontSize: 10, color: '#5a5a6a', marginTop: 2 }}>{entry.date}</div>
        </div>
        ...
        {/* Loot */}
        {entry.lootName && (
          <div style={{ fontSize: 11.5, color: rc.color, background: rc.color + '12', border: `1px solid ${rc.color}33`, borderRadius: 8, padding: '6px 10px', marginBottom: 14, textAlign: 'center' }}>
            🎁 {entry.lootName}
          </div>
        )}
...
const RaidsView = React.memo(function RaidsView({ raids, currentClass, characterName, onStartRaid, onJoinRaid, onSetReady, onLaunchRaid, onAddContribution, onResolve, onLeave, onCloseRaid, onRefresh, guildMembers, onCancelRaid, raidArchive, unomieDebuff, raidSetProgress, raidTrophiesReceived }) {
  ...
  return (
    <div style={{ animation: 'fadeUp 0.3s ease' }}>
      ...
      {/* Побеждённые боссы исчезают из пула — но ТОЛЬКО не-повторяемые (legendary/mythic).
          Повторяемые rare/epic (REPEATABLE_SET_BOSS_IDS) остаются в списке, пока у игрока не
          собран весь сет по его классу — карточка сама покажет "Сет собран" вместо кнопки
          старта (см. RaidBossCard). Раньше этот фильтр прятал их сразу после первой победы —
          именно поэтому Вепрь и Голем пропадали из пула, хотя за ними ещё можно было идти
          за следующим предметом сета. */}
      {(() => {
        const defeatedBossIds = new Set(
          (raidArchive || [])
            .filter(e => e.result === 'victory' && !REPEATABLE_SET_BOSS_IDS.includes(e.bossId))
            .map(e => e.bossId)
        );
        return [...RAID_BOSSES]
...
const BossLoreModal = React.memo(function BossLoreModal({ boss, onClose, currentClass, raidSetProgress, raidTrophiesReceived, raidArchive }) {
  const rc = RAID_RARITY_COLORS[boss.rarity] || RAID_RARITY_COLORS.rare;

  // Следующий предмет сета, который выпадет игроку по его классу — только для повторяемых
  // rare/epic боссов, и только начиная со ВТОРОЙ победы (первая всегда даёт Трофей — см.
  // раздел ниже). Если не в списке (legendary/mythic) — блок просто не показываем.
  const classId = currentClass ? (currentClass.combo ? currentClass.classA.id : currentClass.id) : null;
  const isSetBoss = REPEATABLE_SET_BOSS_IDS.includes(boss.id);
  const trophyReceived = hasReceivedTrophy(boss.id, raidTrophiesReceived, raidArchive);
  const trophyData = classId ? RAID_LOOT_BY_CLASS[boss.id]?.[classId] : null;
  const setItems = isSetBoss && classId ? (RAID_LOOT_SETS_BY_CLASS[boss.id]?.[classId] || []) : [];
  const claimed = (raidSetProgress && raidSetProgress[boss.id]) || [];
  const nextItem = setItems.find((it) => !claimed.includes(it.itemId));
  const setComplete = isSetBoss && setItems.length > 0 && claimed.length >= setItems.length;

  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.88)', zIndex: 90, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}
      onClick={onClose}>
      <div style={{ background: '#1c1c22', border: `1.5px solid ${rc.color}44`, borderRadius: 16, padding: 20, width: '100%', maxWidth: 400, maxHeight: '85vh', overflowY: 'auto' }}
        onClick={(e) => e.stopPropagation()}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: rc.color, textTransform: 'uppercase', letterSpacing: 0.8 }}>{boss.subtitle}</div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#5a5a62', fontSize: 20, lineHeight: 1 }}>✕</button>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 16 }}>
          {boss.image
            ? <img src={boss.image} alt="" style={{ width: 140, height: 140, borderRadius: 20, objectFit: 'cover', border: `1.5px solid ${rc.color}55` }} />
            : <span style={{ fontSize: 90 }}>{boss.creature}</span>}
        </div>

        <div style={{ textAlign: 'center', marginBottom: 4 }}>
          <div style={{ fontSize: 15, fontWeight: 800, color: rc.color, letterSpacing: 0.3 }}>{boss.name}</div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginBottom: 16, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 10.5, color: '#8a8a92' }}>{boss.durationDays} дн.</span>
          <span style={{ fontSize: 10.5, color: '#4a4a52' }}>·</span>
          <span style={{ fontSize: 10.5, fontWeight: 700, color: '#e08a8a', background: '#2a1212', border: '1px solid #5a2a2a', borderRadius: 6, padding: '2px 8px' }}>
            Слабость: {boss.weakness}
          </span>
        </div>

        {/* Трофей / Лут сета — только для повторяемых rare/epic боссов. Первая победа даёт
            Трофей (одноразовый предмет), сет начинает собираться со второй победы. */}
        {isSetBoss && (
          <div style={{ borderTop: `1px solid ${rc.color}22`, paddingTop: 14, marginBottom: 14 }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: rc.color, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 8 }}>
              {trophyReceived ? '🎁 Лут сета' : '🏆 Трофей за первую победу'}
            </div>
            {!trophyReceived ? (
              trophyData ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#15151a', border: '1px solid #24242c', borderRadius: 10, padding: 10 }}>
                  <LootIcon item={trophyData} rarity={boss.rarity} size={44} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 12.5, fontWeight: 700, color: '#f0f0f4' }}>{trophyData.name}</div>
                    <div style={{ fontSize: 10.5, color: '#6a6a72', marginTop: 1 }}>Выпадет за эту победу · сбор сета начнётся со следующей</div>
                  </div>
                </div>
              ) : (
                <div style={{ fontSize: 11.5, color: '#6a6a72' }}>Выбери класс, чтобы увидеть, что выпадет.</div>
              )
            ) : setComplete ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#132a1f', border: '1px solid #2a5a3a', borderRadius: 10, padding: 10 }}>
                <span style={{ fontSize: 18 }}>✅</span>
                <span style={{ fontSize: 12, color: '#7de8a8', fontWeight: 700 }}>Сет собран полностью ({claimed.length}/{setItems.length})</span>
              </div>
            ) : nextItem ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#15151a', border: '1px solid #24242c', borderRadius: 10, padding: 10 }}>
                <LootIcon item={nextItem} rarity={boss.rarity} size={44} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 12.5, fontWeight: 700, color: '#f0f0f4' }}>{nextItem.name}</div>
                  <div style={{ fontSize: 10.5, color: '#6a6a72', marginTop: 1 }}>Выпадет в этот раз · {claimed.length}/{setItems.length} собрано</div>
                </div>
              </div>
            ) : (
              <div style={{ fontSize: 11.5, color: '#6a6a72' }}>Выбери класс, чтобы увидеть, что выпадет.</div>
        ...
        {boss.history && (
          <div style={{ borderTop: `1px solid ${rc.color}22`, paddingTop: 14, marginBottom: 14 }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: rc.color, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 8 }}>📜 История</div>
            <div style={{ fontSize: 12.5, color: '#c0c0ca', lineHeight: 1.7 }}>{boss.history}</div>
          </div>
        )}

        {/* Условие рейда */}
        <div style={{ background: '#15151a', border: '1px solid #24242c', borderRadius: 10, padding: 12 }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: '#8a8a92', textTransform: 'uppercase', letterSpacing: 0.6, marginBottom: 4 }}>Условие рейда</div>
          <div style={{ fontSize: 11.5, color: '#9a9aa2', lineHeight: 1.5 }}>{boss.description}</div>
        </div>
      </div>
    </div>
  );
...
const RaidBossCard = React.memo(function RaidBossCard({ boss, raid, currentClass, characterName, onStart, onJoin, onReady, onLaunch, onAddContribution, onResolve, onLeave, onCloseRaid, guildMembers, onCancelRaid, raidSetProgress, raidTrophiesReceived, raidArchive }) {
  ...
  // Статус сбора сета — считаем один раз, используем и в бейджике на карточке, и в кнопке ниже.
  const setClassId = currentClass ? (currentClass.combo ? currentClass.classA.id : currentClass.id) : null;
  const isSetBoss = REPEATABLE_SET_BOSS_IDS.includes(boss.id);
  const setItemsForCard = isSetBoss && setClassId ? (RAID_LOOT_SETS_BY_CLASS[boss.id]?.[setClassId] || []) : [];
  const setClaimedForCard = (raidSetProgress && raidSetProgress[boss.id]) || [];
  const setTrophyReceived = hasReceivedTrophy(boss.id, raidTrophiesReceived, raidArchive);
  const setComplete = isSetBoss && setItemsForCard.length > 0 && setClaimedForCard.length >= setItemsForCard.length;
  ...
  if (raid && boss.condition.type === 'shared_total') {
    progressValue = countSharedProgress(raid.contributions, boss.condition);
    ...
    <div style={{ border: `1.5px solid ${rc.border}`, background: rc.bg, borderRadius: 16, marginBottom: 10, position: 'relative', overflow: 'hidden' }}>
      ...
      <button onClick={() => setCollapsed(!collapsed)} style={{
        ...
        <div
          ...
          onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); e.stopPropagation(); setShowLore(true); } }}
          aria-label={`О боссе: ${boss.name}`}
          style={{ cursor: 'pointer', flexShrink: 0, borderRadius: 10, lineHeight: 0 }}
        >
          {boss.image
            ? <img src={boss.image} alt="" style={{ width: 40, height: 40, borderRadius: 10, objectFit: 'cover', display: 'block' }} />
            : (boss.creature && <span style={{ fontSize: 22 }}>{boss.creature}</span>)}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ fontSize: 12, fontWeight: 800, color: rc.color, letterSpacing: 0.3 }}>{boss.name}</span>
            {/* Иконка статуса сбора сета — только для повторяемых rare/epic боссов */}
            {isSetBoss && (
              <span title={setComplete ? 'Сет собран' : !setTrophyReceived ? 'Сначала трофей за первую победу' : `Сет: ${setClaimedForCard.length}/${setItemsForCard.length}`} style={{
                display: 'inline-flex', alignItems: 'center', gap: 3, fontSize: 9, fontWeight: 800,
                padding: '1px 6px', borderRadius: 6, flexShrink: 0,
                background: setComplete ? '#132a1f' : '#1e1e26',
                color: setComplete ? '#7de8a8' : '#9a9aa2',
                border: `1px solid ${setComplete ? '#2a5a3a' : '#2a2a32'}`,
              }}>
                {setComplete ? '✅' : !setTrophyReceived ? '🏆' : `🎁 ${setClaimedForCard.length}/${setItemsForCard.length}`}
              </span>
            )}
          </div>
      ...
      {/* Трофей за первую победу — показывается только здесь, до его получения. Полный набор
          сета (иконки всех предметов) — смотри в описании босса (BossLoreModal), не тут, чтобы
          не дублировать один и тот же контент в двух местах. */}
      {isSetBoss && !setTrophyReceived && (() => {
        const trophyDataForCard = setClassId ? RAID_LOOT_BY_CLASS[boss.id]?.[setClassId] : null;
        return (
          <div style={{ background: '#15151a', border: `1px solid ${rc.color}33`, borderRadius: 12, padding: 12, marginBottom: 12 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: rc.color, marginBottom: 8 }}>🏆 Трофей за первую победу</div>
            {trophyDataForCard ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <LootIcon item={trophyDataForCard} rarity={boss.rarity} size={40} />
                <div style={{ fontSize: 11.5, color: '#c0c0ca' }}>{trophyDataForCard.name}</div>
              </div>
            ) : (
              <div style={{ fontSize: 11, color: '#6a6a72' }}>Выбери класс, чтобы увидеть трофей.</div>
            )}
          </div>
        );
      })()}
      ...
      {/* Loot preview — только для НЕ-повторяемых боссов (legendary/mythic). Для повторяемых
          rare/epic смотри блок трофея/сета выше — это разные вещи, не должны дублироваться. */}
      {!status && loot && !isSetBoss && (
      ...
      {!status && (
        setComplete ? (
          <div style={{ width: '100%', padding: '10px', borderRadius: 10, textAlign: 'center', background: '#132a1f', border: '1.5px solid #2a5a3a', fontSize: 12.5, fontWeight: 700, color: '#7de8a8' }}>
            ✅ Сет собран — весь лут этого класса получен
          </div>
        ) : (
      ...
      {(status === 'victory' || status === 'defeat') && (
        raid.closedBy?.includes(characterName) ? (
          <div style={{ width: '100%', padding: '8px', borderRadius: 8, textAlign: 'center', fontSize: 11, color: '#4a4a52' }}>
            Закрыто у тебя — ждём остальных ({raid.closedBy.length}/{raid.participants.length})
          </div>
        ) : (
...
const CosmeticsSection = React.memo(function CosmeticsSection({ purchasedFrameIds, equippedAvatarFrame, onBuyFrame, onEquipFrame, currencyBalance, ownedBackgrounds, onBuyBackground }) {
  ...
  const [openBgTier, setOpenBgTier] = useState(null);
  const bgTierGroups = ['basic', 'standard', 'advanced', 'premium', 'exclusive'];
  const bgTierColors = {
    basic:     { color: '#a0a0b0', label: 'Базовые' },
    standard:  { color: '#5b9bf0', label: 'Стандартные' },
    advanced:  { color: '#9a6ae0', label: 'Продвинутые' },
    premium:   { color: '#f0d272', label: 'Премиум' },
    exclusive: { color: '#e05f9c', label: 'Эксклюзивные' },
  };
  const tierColors = {
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: 20, paddingTop: 16, borderTop: '1px solid #22222e' }}>
        ...
        {bgTierGroups.map(tier => {
          const bgs = PROFILE_BACKGROUNDS.filter(b => b.tier === tier);
          if (!bgs.length) return null;
          const tc = bgTierColors[tier];
          const isOpen = openBgTier === tier;
          const ownedInGroup = bgs.filter(b => (ownedBackgrounds || []).includes(b.id)).length;
          return (
            <div key={tier} style={{ marginBottom: 8 }}>
              <button
                onClick={() => setOpenBgTier(isOpen ? null : tier)}
                style={{
                  width: '100%', display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer',
                  background: isOpen ? tc.color + '14' : '#1a1a22', border: '1px solid ' + (isOpen ? tc.color + '44' : '#28282f'),
                  borderRadius: isOpen ? '10px 10px 0 0' : 10, padding: '10px 14px',
              ...
              >
                <span style={{ fontSize: 10, fontWeight: 800, color: tc.color, textTransform: 'uppercase', letterSpacing: 0.8 }}>{tc.label}</span>
                <span style={{ fontSize: 10, color: '#6a6a72' }}>{ownedInGroup}/{bgs.length}</span>
                <ChevronDown size={13} color="#9a9aa2" style={{ marginLeft: 'auto', transform: isOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }} />
              </button>
              {isOpen && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, border: '1px solid ' + tc.color + '44', borderTop: 'none', borderRadius: '0 0 10px 10px', padding: 10, animation: 'fadeUp 0.15s ease' }}>
                {bgs.map(bg => {
                  ...
                  return (
                    <div key={bg.id} style={{
                      display: 'flex', alignItems: 'center', gap: 10, borderRadius: 10, padding: '8px 12px',
                      background: owned ? tc.color + '14' : '#1a1a22', border: `1px solid ${owned ? tc.color + '55' : '#2a2a32'}`,
                    }}>
                      {/* Превью фона */}
                      <div style={{ width: 56, height: 32, borderRadius: 7, flexShrink: 0, overflow: 'hidden', position: 'relative', background: bg.gradient }}>
                        {bg.image && (
                          <img src={bg.image} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }}
                            onError={(e) => { e.currentTarget.style.display = 'none'; }} />
                        )}
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {!owned && (
                        <button onClick={() => onBuyBackground(bg.id)} disabled={!canAfford} style={{
                          ...
                          background: canAfford ? tc.color + '22' : '#1a1a22', border: '1px solid ' + (canAfford ? tc.color + '66' : '#2a2a32'),
                          color: canAfford ? tc.color : '#4a4a52', cursor: canAfford ? 'pointer' : 'not-allowed', opacity: canAfford ? 1 : 0.5,
                        }}>
                          {bg.price} 💎
                        </button>
                      )}
...
const CharacterView = React.memo(function CharacterView({ statTotals, currentClass, equippedShopItems, purchasedItemIds, onEquip, onSell, earnedTitles, activeTitle, setActiveTitle, titleCooldownDays, levelState, lockedClassId, chosenPathId, achievementsEvaluated, purchasedFrameIds, equippedAvatarFrame, onEquipFrame, avatarEmoji, onSelectAvatarEmoji, shopShields, lastShopShieldUse, activeShield, onUseShopShield, onOpenRecordsWall, ownedBackgrounds, activeBackground, onBuyBackground, onEquipBackground, currencyBalance, polishedItemIds, itemBonusOverrides, onPolishItem, onReforgeItem, getEffectiveBonus, raidSetProgress, raidTrophiesReceived, raidArchive }) {
    ...
    <div style={{ animation: 'fadeUp 0.3s ease' }}>
                  ...
                  {ownedBackgrounds.map(bgId => {
                    ...
                    return (
                      <div key={bgId} style={{
                        ...
                        background: isEq ? '#f0d27214' : '#1a1a22', border: isEq ? '2px solid #f0d272' : '1px solid #28282f',
                      }}>
                        <div style={{ width: 56, height: 32, borderRadius: 7, flexShrink: 0, overflow: 'hidden', position: 'relative', background: bg.gradient }}>
                          {bg.image && (
                            <img src={bg.image} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }}
      ...
      <div style={{
        ...
        backgroundImage: activeBackground
          ? `url(${PROFILE_BACKGROUNDS.find(b => b.id === activeBackground)?.image}), ${PROFILE_BACKGROUNDS.find(b => b.id === activeBackground)?.gradient}`
          : 'none',
        backgroundSize: 'cover', backgroundPosition: 'center',
        borderRadius: 14, padding: activeBackground ? 14 : 0, marginBottom: 4,
      }}>
        {activeBackground && (
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(10,10,13,0.25) 0%, rgba(10,10,13,0.55) 100%)' }} />
        )}
      ...
      {/* Рейдовые сеты — прогресс сбора лута с повторяемых rare/epic боссов, по классу игрока */}
      <SectionLabel text="Рейдовые сеты" style={{ marginTop: 24 }} />
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {REPEATABLE_SET_BOSS_IDS.map((bossId) => {
          const boss = RAID_BOSSES.find((b) => b.id === bossId);
          if (!boss) return null;
          const classId = currentClass ? (currentClass.combo ? currentClass.classA.id : currentClass.id) : null;
          const setItems = classId ? (RAID_LOOT_SETS_BY_CLASS[bossId]?.[classId] || []) : [];
          if (setItems.length === 0) return null;
          const claimed = (raidSetProgress && raidSetProgress[bossId]) || [];
          const trophyReceived = hasReceivedTrophy(bossId, raidTrophiesReceived, raidArchive);
          const rc = RAID_RARITY_COLORS[boss.rarity] || RAID_RARITY_COLORS.rare;
          const complete = claimed.length >= setItems.length;
          return (
            <div key={bossId} style={{ background: '#15151a', border: `1px solid ${rc.color}33`, borderRadius: 12, padding: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                {boss.image
                  ? <img src={boss.image} alt="" style={{ width: 28, height: 28, borderRadius: 7, objectFit: 'cover' }} />
                  : <span style={{ fontSize: 20 }}>{boss.creature}</span>}
                <span style={{ fontSize: 12.5, fontWeight: 700, color: rc.color, flex: 1 }}>{boss.name}</span>
                {!trophyReceived ? (
                  <span style={{ fontSize: 10.5, fontWeight: 800, padding: '2px 8px', borderRadius: 6, background: '#1e1e26', color: '#9a9aa2', border: '1px solid #2a2a32' }}>
                    🏆 сначала трофей
                  </span>
                ) : (
                  <span style={{
                    fontSize: 10.5, fontWeight: 800, padding: '2px 8px', borderRadius: 6,
                    background: complete ? '#132a1f' : '#1e1e26',
                    color: complete ? '#7de8a8' : '#9a9aa2',
                    border: `1px solid ${complete ? '#2a5a3a' : '#2a2a32'}`,
                  }}>
                    {complete ? `✅ ${claimed.length}/${setItems.length}` : `${claimed.length}/${setItems.length}`}
                  </span>
                )}
              </div>
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {setItems.map((it) => {
                  const has = claimed.includes(it.itemId);
                  return (
                    <div key={it.itemId} title={it.name}>
                      <LootIcon item={it} rarity={boss.rarity} size={40} locked={!has} />
                    </div>
                  );
                })}
              </div>
...
const EquippedGear = React.memo(function EquippedGear({ equippedShopItems, lockedClassId, classColor, onSell, onSlotTap }) {
  ...
  function renderSlotSquare(slot) {
    ...
    const lootFrame = item?.raidLoot ? (LOOT_FRAME_BY_RARITY[item.rarity] || LOOT_FRAME_BY_RARITY.rare) : null;
      ...
      <React.Fragment key={slot.key}>
        <button
          ...
          style={{
            ...
            borderRadius: '14%', border: (lootFrame?.glow ? '3px' : '2px') + ' solid ' + (rarity ? rarity.color + '99' : '#3a3a42'),
            background: rarity ? rarity.color + '26' : 'rgba(20,20,26,0.55)',
            // Лут с боссов легендарной/мифической редкости получает лёгкое свечение рамки —
            // единый стандарт для рейдового лута (см. LOOT_FRAME_BY_RARITY), не только у новых
            // присланных вручную иконок без собственной рамки, но и у любого будущего лута.
            boxShadow: lootFrame?.glow && active ? `0 0 10px ${rarity.color}77` : 'none',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
  ...
  return (
    <div style={{
      ...
      {lockedClassId && (
        <div
          style={{
            ...
            backgroundImage: `url('/character/backgrounds/${lockedClassId}.webp')`,
            backgroundSize: 'cover', backgroundPosition: 'center',
            pointerEvents: 'none',
          }}
        >
      ...
      <div style={{ position: 'relative', zIndex: 1, width: '100%', paddingTop: `${(903 / 520) * 100}%` }}>
        <img
      ...
      <div style={{ position: 'relative', zIndex: 1, textAlign: 'center', fontSize: 10.5, color: '#5a5a62', marginTop: 6 }}>
        Нажми на слот, чтобы выбрать предмет
      </div>

      {(bonusEntries.length > 0 || equippedCrystalPct > 0 || statEntries.length > 0) ? (
        <div style={{ ...styles.equippedBonusSummary, position: 'relative', zIndex: 1, background: '#15151a', border: '1px solid #24242c', borderRadius: 12, padding: 14, marginTop: 4 }}>
        ...
        <div style={{ ...styles.equippedEmptyHint, position: 'relative', zIndex: 1 }}>Зайди в «Магазин», чтобы купить и надеть снаряжение</div>
      )}
      {!allSlotsActive && activeEquippedItems.length > 0 && (
        <div style={{ position: 'relative', zIndex: 1, fontSize: 11, color: '#5a5a62', textAlign: 'center', marginTop: 8 }}>
          Заполни все 6 слотов активной экипировкой → +15% XP ко всему ({SHOP_SLOTS.length - activeEquippedItems.length} осталось)
        </div>
      )}
      {hasInactiveItem && (
        <div style={{ position: 'relative', zIndex: 1, fontSize: 11, color: '#e08a8a', textAlign: 'center', marginTop: 6 }}>
      ...
      {chosenPath && (
        <div style={{
          ...
          <div style={{
            ...
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            background: chosenPath.color + '22', overflow: 'hidden',
          }}>
            {chosenPath.icon
              ? <img src={chosenPath.icon} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              : <Footprints size={20} color={chosenPath.color} />}
          </div>
      ...
      {showRitualSection && (
        <>
          ...
          <div style={{ ...styles.historyRow, display: 'flex', alignItems: 'center' }}>
            <div style={{ ...styles.activityIconWrap, background: '#c9a8f522', color: '#c9a8f5', width: 32, height: 32, position: 'relative', overflow: 'hidden' }}>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.9)', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}
      ...
      <div style={{
        ...
        backgroundImage: member.activeBackground
          ? `url(${PROFILE_BACKGROUNDS.find(b => b.id === member.activeBackground)?.image}), ${PROFILE_BACKGROUNDS.find(b => b.id === member.activeBackground)?.gradient}`
          : 'none',
        backgroundColor: member.activeBackground ? undefined : '#13131a',
        backgroundSize: 'cover', backgroundPosition: 'center',
        width: '100%', maxWidth: 420, margin: '24px auto', borderRadius: 16, padding: '0 0 20px', animation: 'fadeUp 0.25s ease',
      }}>
        {member.activeBackground && (
          <div style={{ position: 'absolute', inset: 0, borderRadius: 16, background: 'linear-gradient(180deg, rgba(10,10,13,0.3) 0%, rgba(10,10,13,0.6) 100%)', pointerEvents: 'none' }} />
        )}
        <div style={{ position: 'relative' }}>
...
function computeRaidStats(boss, raid) {
  ...
  // Урон по участникам — для "дневных" боссов (dedupeByDate) считаем уникальные дни на
  // игрока, а не сырое число логов, той же логикой, что и общий прогресс (countSharedProgress).
  const damageByPlayer = {};
  participants.forEach(p => { damageByPlayer[p.name] = 0; });
  if (cond.dedupeByDate) {
    participants.forEach(p => {
      const uniqueDates = new Set(contribs.filter(c => c.participantName === p.name).map(c => c.date));
      damageByPlayer[p.name] = uniqueDates.size;
    });
  } else {
    contribs.forEach(c => {
  ...
  if (cond.type === 'shared_total' || cond.type === 'shared_count') {
    target = cond.target;
    totalDone = countSharedProgress(contribs, cond);
    participants.forEach(p => {
  ...
  const allMembers = [
    {
      ...
      activeBackground: myActiveBackground || null,
    },
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {sorted.map((member) => {
          ...
          return (
            <div
              ...
              style={{
                ...
                backgroundImage: member.activeBackground
                  ? `url(${PROFILE_BACKGROUNDS.find(b => b.id === member.activeBackground)?.image}), ${PROFILE_BACKGROUNDS.find(b => b.id === member.activeBackground)?.gradient}`
                  : 'none',
                backgroundColor: member.activeBackground ? undefined : (classTintBg || (isYou ? '#1c1e14' : '#1c1c22')),
                backgroundSize: 'cover', backgroundPosition: 'center',
            ...
            >
              {member.activeBackground && (
                <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(10,10,13,0.35) 0%, rgba(10,10,13,0.7) 100%)', pointerEvents: 'none' }} />
              )}
    ...
    return (
      <div style={{ background: '#1c1c22', border: `1.5px solid ${catColor}44`, borderRadius: 14, padding: '12px 14px', marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
          <div style={{ width: 44, height: 44, borderRadius: 10, background: catColor + '22', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', position: 'relative' }}>
      ...
      <div>
        ...
        <div style={{ background: '#1c1c22', border: '1.5px solid ' + (catDef?.color || '#e0a868') + '44', borderRadius: 16, padding: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
            <div style={{ width: 44, height: 44, borderRadius: 11, background: (catDef?.color || '#e0a868') + '22', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', position: 'relative' }}>
        ...
        {filtered.map(ch => {
          ...
          return (
            <button key={ch.id} onClick={() => setDetailId(ch.id)} style={{
              ...
              <div style={{ width: 52, height: 52, borderRadius: 12, background: (catDef?.color || '#e0a868') + '18', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden', position: 'relative' }}>
                <CIcon size={22} color={catDef?.color || '#e0a868'} />
                {ch.icon && (
                  <img src={ch.icon} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }}
                    onError={(e) => { e.currentTarget.style.display = 'none'; }} />
                )}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
