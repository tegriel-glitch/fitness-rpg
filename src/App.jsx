...
function AppInner() {
  ...
  useEffect(() => {
    ...
    morningRitualLog, shopShields, lastShopShieldUse, lastRaidShieldUse, raidArchive, unomieDebuff, raidSetProgress,
  ...
  const statTotals = useMemo(() => {
    ...
    // Deduplicate by bossId (same boss can only grant stat bonus once) — this legacy path only
    // still applies to legendary/mythic (RAID_LOOT_BY_CLASS), which aren't part of the
    // repeatable-set rework below.
    const seenBossIds = new Set();
    allVictoryRaids.forEach(({ bossId, startClassId }) => {
      if (REPEATABLE_SET_BOSS_IDS.includes(bossId)) return; // handled per-item below instead
  ...
  useEffect(() => {
    Object.entries(raids).forEach(([bossId, raid]) => {
      ...
      const claimedSoFar = raidSetProgress[bossId] || [];
      // Хроника/архивная запись создаётся только один раз — на самую первую победу над этим
      // боссом. Повторные прохождения (за следующим предметом сета) не плодят новые записи в
      // архиве — только тост о полученном предмете (см. эффект наград ниже).
      const shouldArchive = !isRepeatable || claimedSoFar.length === 0;

      if (shouldArchive) {
        const classId = raid.startClassId || effectiveClassId || 'pathfinder';
        const lootData = RAID_LOOT_BY_CLASS[bossId]?.[classId];
        const setItems = RAID_LOOT_SETS_BY_CLASS[bossId]?.[classId] || [];
        const nextSetItem = setItems.find((it) => !claimedSoFar.includes(it.itemId));
        const lootName = isRepeatable ? (nextSetItem?.name || null) : (lootData?.name || null);
  ...
  }, [raids, characterName, effectiveClassId, lockedClassId, currentClass, guildMembers, raidSetProgress]);
  ...
  }, [raids, characterName, effectiveClassId, raidSetProgress]);
...
const RaidsView = React.memo(function RaidsView({ raids, currentClass, characterName, onStartRaid, onJoinRaid, onSetReady, onLaunchRaid, onAddContribution, onResolve, onLeave, onCloseRaid, onRefresh, guildMembers, onCancelRaid, raidArchive, unomieDebuff, raidSetProgress }) {
...
const BossLoreModal = React.memo(function BossLoreModal({ boss, onClose, currentClass, raidSetProgress }) {
  ...
  // rare/epic боссов. Если не в списке (legendary/mythic) — блок просто не показываем.
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.88)', zIndex: 90, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}
      ...
      <div style={{ background: '#1c1c22', border: `1.5px solid ${rc.color}44`, borderRadius: 16, padding: 20, width: '100%', maxWidth: 400, maxHeight: '85vh', overflowY: 'auto' }}
        ...
        {/* Лут сета — только для повторяемых rare/epic боссов */}
        {isSetBoss && (
          <div style={{ borderTop: `1px solid ${rc.color}22`, paddingTop: 14, marginBottom: 14 }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: rc.color, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 8 }}>🎁 Лут сета</div>
            {setComplete ? (
...
const RaidBossCard = React.memo(function RaidBossCard({ boss, raid, currentClass, characterName, onStart, onJoin, onReady, onLaunch, onAddContribution, onResolve, onLeave, onCloseRaid, guildMembers, onCancelRaid, raidSetProgress }) {
...
const CharacterView = React.memo(function CharacterView({ statTotals, currentClass, equippedShopItems, purchasedItemIds, onEquip, onSell, earnedTitles, activeTitle, setActiveTitle, titleCooldownDays, levelState, lockedClassId, chosenPathId, achievementsEvaluated, purchasedFrameIds, equippedAvatarFrame, onEquipFrame, avatarEmoji, onSelectAvatarEmoji, shopShields, lastShopShieldUse, activeShield, onUseShopShield, onOpenRecordsWall, ownedBackgrounds, activeBackground, onBuyBackground, onEquipBackground, currencyBalance, polishedItemIds, itemBonusOverrides, onPolishItem, onReforgeItem, getEffectiveBonus, raidSetProgress }) {
