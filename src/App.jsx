...
function AppInner() {
  ...
  function computeChallengeProgress(ch, logsAll, passiveLogsAll) {
    ...
    // Все клятвы заканчиваются ТОЛЬКО по истечении срока.
    // Пропуски фиксируются визуально, но провал наступает только когда время вышло
    // и цель не выполнена (isComplete === false). Ни одна клятва не проваливается досрочно.
    const isFailed = elapsed >= def.duration && !isComplete;
