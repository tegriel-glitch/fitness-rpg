...
function AchievementsView({ achievements, pentaResults, comboCounts, balanceCounts, secretAchievements, mythicAchievements, bestiaryAchievements, challengeAchievements, roadStoryAchievements }) {
  const byId = {};
...
function CharacterView({ statTotals, currentClass, equippedShopItems, purchasedItemIds, onEquip, earnedTitles, activeTitle, setActiveTitle, titleCooldownDays, levelState, lockedClassId, chosenPathId, achievementsEvaluated, purchasedFrameIds, equippedAvatarFrame, onEquipFrame, avatarEmoji, onSelectAvatarEmoji, shopShields, lastShopShieldUse, activeShield, onUseShopShield, onOpenRecordsWall, ownedBackgrounds, activeBackground, onBuyBackground, onEquipBackground, currencyBalance, polishedItemIds, itemBonusOverrides, onPolishItem, onReforgeItem, getEffectiveBonus }) {
  ...
  return (
    <div style={{ animation: 'fadeUp 0.3s ease' }}>
      ...
      {/* HALL OF FAME — сразу под Стеной подвигов */}
      {achievementsEvaluated && (() => {
        ...
        return unlockedTiers.length > 0 ? <div style={{ marginTop: 8 }}><HallOfFameButton unlockedTiers={unlockedTiers} /></div> : null;
      })()}
      ...
      <div style={{ marginTop: 20 }}>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <Palette size={16} color="#f0d272" />
            <span style={{ fontWeight: 700, fontSize: 13 }}>Фоны профиля</span>
            <span style={{ fontSize: 11, color: '#6a6a72' }}>{(ownedBackgrounds || []).length}/{PROFILE_BACKGROUNDS.length}</span>
          </div>
          <ChevronDown size={16} color="#9a9aa2" style={{ transform: bgOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }} />
        </button>
        {bgOpen && (
          <div style={{ animation: 'fadeUp 0.2s ease', background: '#18181f', border: '1.5px solid #28282f', borderTop: 'none', borderRadius: '0 0 12px 12px', padding: 14, display: 'flex', flexDirection: 'column', gap: 8 }}>
            {PROFILE_BACKGROUNDS.map(bg => {
              ...
              return (
                <div key={bg.id} style={{
                  ...
                  background: bg.gradient, border: active ? '2px solid #f0d272' : '1px solid #28282f',
                }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: '#f0f0f4' }}>{bg.name}</div>
                    {!owned && <div style={{ fontSize: 10, color: '#c0c0ca' }}>💎 {bg.price}</div>}
                  </div>
                  {owned ? (
                    <button onClick={() => onEquipBackground(bg.id)} style={{
                      fontSize: 11, fontWeight: 700, padding: '6px 12px', borderRadius: 8,
                      border: '1px solid ' + (active ? '#f0d272' : '#5a5a62'), background: active ? '#f0d27222' : 'transparent',
                      color: active ? '#f0d272' : '#c0c0ca', cursor: 'pointer',
                    }}>
                      {active ? 'Снять' : 'Выбрать'}
                    </button>
                  ) : (
                    <button onClick={() => onBuyBackground(bg.id)} disabled={currencyBalance < bg.price} style={{
                      fontSize: 11, fontWeight: 700, padding: '6px 12px', borderRadius: 8,
                      border: '1px solid #f0d272', background: 'transparent', color: '#f0d272',
                      cursor: currencyBalance < bg.price ? 'not-allowed' : 'pointer', opacity: currencyBalance < bg.price ? 0.5 : 1,
                    }}>
                      Купить
                    </button>
                  )}
    ...
    <div style={{ animation: 'fadeUp 0.3s ease' }}>
      ...
      <RulesSection id="update" icon="🆕" title="Апдейты" color="#4ce0c0" open={open === 'update'} onToggle={toggle}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14, padding: '8px 12px', background: '#0f2a1a', border: '1px solid #2a5a3a', borderRadius: 10 }}>
          <span style={{ fontSize: 18 }}>⚡</span>
          <div>
            <div style={{ fontSize: 12, fontWeight: 800, color: '#4ce0c0' }}>Версия 1.8 — Магазин, расходники и второй вызов</div>
            <div style={{ fontSize: 10.5, color: '#5a8a72' }}>Больше снаряжения, новые способы им управлять</div>
          </div>
        </div>
        ...
        <Li><B>🖼 Фоны профиля</B> — 5 фонов на выбор (Горы, Лес, Додзё, Библиотека, Звёздное небо), покупаются навсегда и переключаются в «Персонаж».</Li>
        ...
        <Li><B>📬 Мелкие удобства</B> — активные баффы от дорожных историй теперь видны прямо на «Прогрессе»; Доска почёта переехала под Стену подвигов; длинные сообщения в почте больше не вылезают за край экрана на телефоне.</Li>
        <Note>Как обычно, если что-то выглядит странно — пишите, разберёмся.</Note>

        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 18, marginBottom: 14, padding: '8px 12px', background: '#0f2a1a', border: '1px solid #2a5a3a', borderRadius: 10 }}>
          <span style={{ fontSize: 18 }}>⚡</span>
          <div>
            <div style={{ fontSize: 12, fontWeight: 800, color: '#4ce0c0' }}>Версия 1.7 — Существа, подвиги и почта</div>
            <div style={{ fontSize: 10.5, color: '#5a8a72' }}>Мир стал больше: 73 существа, стена рекордов, живая почта</div>
          </div>
        </div>
        ...
        <Li><B>🏆 Стена подвигов</B> — личные рекорды по 21 категории (макс. дистанция, лучшие серии, суммы и т.д.), никогда не сбрасываются даже если серия прервалась. При новом рекорде — модалка с разницей от прошлого значения. Открывается кнопкой в «Персонаж», у товарищей по гильдии — кнопкой «Подвиги» в их карточке (только для просмотра).</Li>
