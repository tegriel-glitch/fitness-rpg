...
export default function App() {
  ...
  const challengeProgress = useMemo(() => {
    ...
    const timeExpired = elapsed >= def.duration;
    const missedSoFar = dayResults.slice(0, elapsed).filter(d => d === false).length;
    const maxAllowed = def.allowedMisses || 0;

    // Fail and complete only trigger AFTER the full duration has elapsed (like a raid timer)
    const isFailed = timeExpired && missedSoFar > maxAllowed;
    const isComplete = timeExpired && !isFailed &&
    ...
    return { def, dayResults, elapsed, missedSoFar, maxAllowed, isFailed, isComplete, perfectDays, timeExpired };
  ...
  function setReady(bossId) {
    ...
    const newRaid = { ...raid, participants: updated, status: newStatus, startDate, activatedAt: allReady ? new Date().toISOString() : raid.activatedAt };
    setRaids((prev) => ({ ...prev, [bossId]: newRaid }));
    persistRaid(bossId, newRaid);
    ...
    <div style={styles.header}>
      ...
      {showProfileModal && (() => {
        ...
        const selfMember = {
          ...
          unlockedSkillLevels: [],
          activeChallenge: challengeState.active ? (() => {
            const def = CHALLENGE_CATALOG.find(c => c.id === challengeState.active.id);
            if (!def) return null;
            const end = new Date(challengeState.active.startDate);
            end.setDate(end.getDate() + def.duration);
            return { name: def.name, daysLeft: Math.max(0, Math.ceil((end - new Date()) / 86400000)) };
          })() : null,
        };
        return (
          <GuildMemberDetail
...
function RaidsView({ raids, currentClass, characterName, onStartRaid, onJoinRaid, onSetReady, onAddContribution, onResolve, onLeave, onCloseRaid, onRefresh, guildMembers, onCancelRaid }) {
...
function RaidBossCard({ boss, raid, currentClass, characterName, onStart, onJoin, onReady, onAddContribution, onResolve, onLeave, onCloseRaid, guildMembers, onCancelRaid }) {
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {member.activeChallenge && (
                <div style={{ marginTop: 6, display: 'inline-flex', alignItems: 'center', gap: 4, background: '#1a1a2e', border: '1px solid #3a3a5a', borderRadius: 6, padding: '3px 8px' }}>
                  <span style={{ fontSize: 10 }}>🔥</span>
                  <span style={{ fontSize: 9.5, color: '#c0a0f0' }}>{member.activeChallenge.name}</span>
                  <span style={{ fontSize: 9, color: '#6a5a82' }}>· {member.activeChallenge.daysLeft}дн</span>
                </div>
              )}
            </div>
...
function GuildView({ playerName, playerCharacterName, playerLevel, playerClass, playerPhysical, playerMental, playerTitleEntry, activeTitle, earnedTitles, raids, guildLikes, onLike, lockedClassId, chosenPathId, guildMembers, onRefreshGuild, onLogout, onTransferCrystals, onGiftItem, myPurchasedItemIds, myEquippedShopItems, myCurrencyBalance, myEquippedAvatarFrame, myAvatarEmoji, onSendMessage }) {
  ...
  function memberFromRow(row) {
    ...
    return {
      ...
      equippedCount,
      activeChallenge: (() => {
        const cs = row.challenge_state;
        if (!cs?.active) return null;
        const def = CHALLENGE_CATALOG.find(c => c.id === cs.active.id);
        if (!def) return null;
        const end = new Date(cs.active.startDate);
        end.setDate(end.getDate() + def.duration);
        return { name: def.name, daysLeft: Math.max(0, Math.ceil((end - new Date()) / 86400000)) };
      })(),
...
function ChallengesView({ challengeState, challengeProgress, activeChallengeBuff, challengeFailDebuff, challengeCooldownDays, challengeAchievements, level, onStart }) {
  ...
  const activeCompact = challengeProgress && challengeState.active ? (() => {
    const { def, dayResults, elapsed, missedSoFar, maxAllowed, isFailed, isComplete, timeExpired } = challengeProgress;
    ...
    // Conditions already satisfied but timer still running
    const conditionsMet = !isFailed && successDays > 0 && def.checkComplete && def.checkComplete(dayResults.slice(0, def.duration), def, []);
    return (
      <div style={{ background: '#1c1c22', border: `1.5px solid ${catColor}44`, borderRadius: 14, padding: '12px 14px', marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'space-between', fontSize: 10 }}>
          ...
          <span style={{ color: missedSoFar > 0 ? '#e0a868' : '#6a6a72' }}>✕ {missedSoFar}/{maxAllowed} пропущено</span>
          <span style={{ color: '#8a8a92' }}>День {elapsed}/{def.duration}</span>
        </div>
        {missedSoFar > 0 && missedSoFar >= maxAllowed && !timeExpired && (
          <div style={{ marginTop: 5, fontSize: 9.5, color: '#e05f4a', textAlign: 'center' }}>
            ⚠️ Допуск исчерпан — больше пропусков нельзя
          </div>
        )}
        <div style={{ display: 'flex', gap: 2, flexWrap: 'wrap', marginTop: 6 }}>
          {dayResults.map((r, i) => (<div key={i} style={{ width: 8, height: 8, borderRadius: 2, background: r === null ? '#25252c' : r ? '#3a8a3a' : '#8a3a3a' }} />))}
        </div>
      </div>
    );
  })() : null;
