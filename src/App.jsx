...
function AppInner() {
  ...
  // chronicle — including its randomly-picked finisher variant — far more often than "once per
  // time you actually open a chronicle", making it look like the text kept regenerating itself.
  const guildRosterWithSelf = React.useMemo(() => (
    [
      ...guildMembers.map(m => ({ name: m.character_name || m.nickname, locked_class_id: m.locked_class_id, characterName: m.character_name })),
      { name: characterName, locked_class_id: lockedClassId, characterName },
    ]
  ), [guildMembers, characterName, lockedClassId]);
  ...
  function closeRaid(bossId) {
    ...
    if (isCompleted && raid.participants?.some(p => p.name === characterName) && !raid.closedBy?.includes(characterName)) {
      ...
      classByName.set(characterName, lockedClassId || classId);
      (guildMembers || []).forEach((m) => {
        const nm = m.character_name || m.nickname;
        if (nm) classByName.set(nm, m.locked_class_id || null);
      });
      const participantClasses = raid.participants.map(p => ({ name: p.name, classId: classByName.get(p.name) || null }));
      // Register (мемуары/сказание/дозорный листок) is rolled once here and frozen into the
      // archive entry — reopening the same archived chronicle later must not reroll it.
      const CHRONICLE_REGISTERS = ['memoir', 'saga', 'gazette'];
      const chronicleRegister = CHRONICLE_REGISTERS[Math.floor(Math.random() * CHRONICLE_REGISTERS.length)];
      const archiveEntry = {
...
const ArchiveDetailModal = React.memo(function ArchiveDetailModal({ entry, onClose, guildMembers }) {
  ...
  const story = React.useMemo(() => {
    ...
    return composeBattleChronicle({
      bossId: entry.bossId, participants, result: entry.result,
      // Older archive entries (before this feature shipped) have no frozen register —
      // pick one on the fly for them; it just won't be stable across repeated opens, which
      // is an acceptable trade-off for pre-existing data rather than crashing.
      register: entry.chronicleRegister || ['memoir', 'saga', 'gazette'][Math.floor(Math.random() * 3)],
    });
  }, [entry, guildMembers]);
