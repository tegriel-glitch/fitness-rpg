...
const RaidBossCard = React.memo(function RaidBossCard({ boss, raid, currentClass, characterName, onStart, onJoin, onReady, onLaunch, onAddContribution, onResolve, onLeave, onCloseRaid, guildMembers, onCancelRaid, raidSetProgress, raidTrophiesReceived }) {
  ...
  return (
    <div style={{ border: `1.5px solid ${rc.border}`, background: rc.bg, borderRadius: 16, marginBottom: 10, position: 'relative', overflow: 'hidden' }}>
      ...
      {/* Start button */}
      {!status && (() => {
        const classIdForGate = currentClass ? (currentClass.combo ? currentClass.classA.id : currentClass.id) : null;
        const isSetBoss = REPEATABLE_SET_BOSS_IDS.includes(boss.id);
        const setItems = isSetBoss && classIdForGate ? (RAID_LOOT_SETS_BY_CLASS[boss.id]?.[classIdForGate] || []) : [];
        const claimed = (raidSetProgress && raidSetProgress[boss.id]) || [];
        const setComplete = isSetBoss && setItems.length > 0 && claimed.length >= setItems.length;
        if (setComplete) {
          return (
