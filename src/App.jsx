...
const ArchiveDetailModal = React.memo(function ArchiveDetailModal({ entry, onClose }) {
  ...
  const story = React.useMemo(() => {
    ...
    // older archive entries fall back gracefully (composeBattleChronicle treats missing
    // classId as a generic "did their part" contribution, no crash, just less specific text).
    const participants = (entry.participantClasses && entry.participantClasses.length > 0)
      ? entry.participantClasses
      : (entry.participants || []).map((name) => ({ name, classId: null }));
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.9)', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}
      ...
      <div style={{
        ...
        {member.activeBackground && (
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(10,10,13,0.3) 0%, rgba(10,10,13,0.6) 100%)', pointerEvents: 'none' }} />
        )}
        <div style={{ position: 'relative' }}>
