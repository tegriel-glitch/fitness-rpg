...
const AVATAR_FRAMES = [
  ...
  // === ЭПИЧЕСКИЕ ===
  { id: 'frame_epic_armor',    name: 'Железная броня',   rarity: 'epic',    price: 60,
    style: { border: '4px solid #8a9ab0', boxShadow: '0 0 0 2px #4a5a6a, 0 0 16px #6a8aaa66' } },
  { id: 'frame_epic_dragon',   name: 'Чешуя дракона',    rarity: 'epic',    price: 67,
...
export default function App() {
  ...
  const [equippedAvatarFrame, setEquippedAvatarFrame] = useState(null);
  const [purchasedFrameIds, setPurchasedFrameIds] = useState([]);
  ...
  function buyAvatarFrame(frameId) {
    ...
    if (frame.requirement?.type === 'mythic') {
      const unlocked = unlockedMythicIds?.has(frame.requirement.id);
  ...
  // Сохраняем current_level при его изменении
  useEffect(() => {
