...
export default function App() {
  ...
  return (
    <div style={styles.app}>
      ...
      {/* Mail Modal */}
      {showMailModal && (
        <MailModal messages={mailMessages} onClose={() => setShowMailModal(false)} onMarkRead={handleMarkRead} onMarkAllRead={handleMarkAllRead} />
      )}
...
function MailModal({ messages, onClose, onMarkRead, onMarkAllRead }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.92)', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}
      onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div style={{ background: '#111118', width: '100%', maxWidth: 460, margin: '0 auto', minHeight: '100%', animation: 'fadeUp 0.25s ease' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 18px', borderBottom: '1px solid #25252c', position: 'sticky', top: 0, background: '#111118', zIndex: 2 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <Mail size={18} color="#4f7cff" />
            <span style={{ fontSize: 16, fontWeight: 800, color: '#f0f0f4' }}>Почта</span>
            {messages.filter(m => !m.read).length > 0 && (
              <span style={{ fontSize: 10, color: '#e05f4a', fontWeight: 700 }}>{messages.filter(m => !m.read).length} новых</span>
            )}
          </div>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            {messages.some(m => !m.read) && (
              <button onClick={onMarkAllRead} style={{ fontSize: 10, color: '#4f7cff', background: 'none', border: '1px solid #4f7cff33', borderRadius: 8, padding: '4px 10px', cursor: 'pointer', fontWeight: 700 }}>Прочитать всё</button>
            )}
            <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4 }}><X size={20} color="#6a6a72" /></button>
          </div>
        </div>
        <div style={{ padding: '8px 12px' }}>
          {messages.length === 0 && <div style={{ textAlign: 'center', padding: 40, color: '#4a4a58', fontSize: 13 }}>Нет сообщений</div>}
          {messages.map(msg => (
            <button key={msg.id} onClick={() => openMessage(msg)} style={{
              display: 'flex', gap: 10, padding: '10px 12px', width: '100%',
              background: msg.read ? 'transparent' : '#1a1a2e',
              border: `1px solid ${msg.read ? '#1e1e26' : '#2a2a4a'}`,
              borderRadius: 10, marginBottom: 6, cursor: 'pointer', textAlign: 'left',
            }}>
              <div style={{ width: 36, height: 36, borderRadius: '50%', flexShrink: 0, background: isSystem(msg) ? '#1a1a3a' : '#1a2a1a', border: `1.5px solid ${isSystem(msg) ? '#4f7cff44' : '#3a5a3a'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>
                {isSystem(msg) ? '🔮' : '👤'}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 2 }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: isSystem(msg) ? '#9a8af5' : '#d0d0da' }}>{senderName(msg)}</span>
                  <span style={{ fontSize: 9, color: '#4a4a58', flexShrink: 0 }}>{new Date(msg.created_at).toLocaleDateString('ru-RU', { day: 'numeric', month: 'short' })}</span>
                </div>
              ...
              {!msg.read && <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#4f7cff', flexShrink: 0, marginTop: 4 }} />}
            </button>
          ))}
        </div>
      </div>
      {selectedMsg && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 1010, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }} onClick={() => setSelectedMsg(null)}>
          <div style={{ background: '#18181e', border: '1.5px solid #2a2a3a', borderRadius: 16, padding: 20, maxWidth: 380, width: '100%', maxHeight: '70vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              <span style={{ fontSize: 20 }}>{isSystem(selectedMsg) ? '🔮' : '👤'}</span>
              <div>
                <div style={{ fontSize: 14, fontWeight: 800, color: isSystem(selectedMsg) ? '#9a8af5' : '#d0d0da' }}>{senderName(selectedMsg)}</div>
                <div style={{ fontSize: 10, color: '#4a4a58' }}>{new Date(selectedMsg.created_at).toLocaleString('ru-RU', { day: 'numeric', month: 'long', hour: '2-digit', minute: '2-digit' })}</div>
              </div>
            </div>
            <div style={{ fontSize: 13, color: '#c0c0cc', lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{selectedMsg.message}</div>
            <button onClick={() => setSelectedMsg(null)} style={{ marginTop: 16, width: '100%', padding: 8, borderRadius: 10, background: '#1a1a2a', border: '1px solid #2a2a3a', color: '#8a8a9a', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>Закрыть</button>
          </div>
        </div>
      )}
    </div>
