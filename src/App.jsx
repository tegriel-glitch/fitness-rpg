...
function AppInner() {
  ...
  //
  // IMPORTANT: `raids` is a single shared Supabase record per boss, not per-player (see the
  // loot-granting effect above). This used to unconditionally delete that shared record the
  // moment ANY one participant clicked "Закрыть" — whoever closed first wiped the raid out
  // from under the other two before their own clients ever got a chance to archive it for
  // themselves. That's exactly how a finished raid could vanish for one player without ever
  // landing in their archive. Fix below: track who has archived it via `closedBy`, only
  // actually delete the shared record once every participant has closed it on their own end.
  function closeRaid(bossId) {
    const raid = raids[bossId];
    const boss = RAID_BOSSES.find(b => b.id === bossId);
    const isCompleted = raid && boss && (raid.status === 'victory' || raid.status === 'defeat');
    // Archive if completed (victory or defeat), player was a participant, and they haven't
    // already archived this exact raid (guards double-clicking before state settles).
    if (isCompleted && raid.participants?.some(p => p.name === characterName) && !raid.closedBy?.includes(characterName)) {
      const classId = raid.startClassId || effectiveClassId || 'pathfinder';
      const lootData = RAID_LOOT_BY_CLASS[bossId]?.[classId];
      // Resolve each participant's classId (self + guild roster) for the battle chronicle —
      // raid.participants only stores {name, ready, role}, role is an activity role, not a class.
      // Falls back to the dynamic (pre-lock) class for anyone under level 10 — otherwise a raid
      // fought before someone locked their class would archive with classId: null for them.
      ...
      // Chronicle text is resolved live from bossId + participantClasses + result — nothing to
      // freeze at close time. (Register variants — мемуары/сказание/дозорный листок — are
      // deprioritized for now; composeBattleChronicle always uses мемуары.)
      const archiveEntry = {
...
const ArchiveDetailModal = React.memo(function ArchiveDetailModal({ entry, onClose, guildMembers }) {
  ...
  const story = React.useMemo(() => {
    ...
    // For older entries, best-effort resolve each participant's CURRENT locked class from the
    // guild roster instead of just giving up with classId: null — classes lock and rarely
    // change once set, so this is usually still accurate for old raids and gives much more
    // specific text than the generic "did their part" fallback.
    let participants;
    if (entry.participantClasses && entry.participantClasses.length > 0) {
      participants = entry.participantClasses;
    } else {
      const classByName = new Map((guildMembers || []).map((m) => [m.name || m.characterName, m.locked_class_id || null]));
      participants = (entry.participants || []).map((name) => ({ name, classId: classByName.get(name) || null }));
    }
