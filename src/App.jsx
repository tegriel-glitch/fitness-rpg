...
import { Flame, Dumbbell, Salad, Moon, BookOpen, Swords, Trophy, Plus, X, Sparkles, TrendingUp, Calendar, ChevronDown, Zap, Pizza, Heart, Brain, FlaskConical, Cookie, Check, Lock, Footprints, Wind, PersonStanding, Music, Users, HeartHandshake, Plane, Trees, Palette, Beef, Sprout, Droplet, Shield, Sword, Star, Crown, Gem, Wand2, Hand, Sparkle, Skull, Pencil, Medal, Scale, Glasses, Watch, Shirt, HardHat, Ear, Axe, Gauge, ShoppingBag, Mail, Compass, Scroll, RotateCcw } from 'lucide-react';

// ---------- CLASS ICON IMAGES (заменяют lucide-иконки классов) ----------
// Положи файлы сюда: src/assets/class-icons/<id>.png (или поменяй путь на свой)
import pathfinderIconImg from './assets/class-icons/pathfinder.png';
import berserkerIconImg from './assets/class-icons/berserker.png';
import battlemasterIconImg from './assets/class-icons/battlemaster.png';
import monkIconImg from './assets/class-icons/monk.png';
import shamanIconImg from './assets/class-icons/shaman.png';
import archmageIconImg from './assets/class-icons/archmage.png';

// Рендерит картинку-иконку класса, если она есть у cls (cls.iconImg), иначе — старую lucide-иконку (cls.icon)
function ClassIcon({ cls, size = 20, color, style }) {
  if (!cls) return null;
  if (cls.iconImg) {
    return (
      <img
        ...
        width={size}
        height={size}
        style={{ borderRadius: Math.max(4, size * 0.22), objectFit: 'cover', imageRendering: 'pixelated', flexShrink: 0, ...style }}
      />
    );
  }
  const Icon = cls.icon;
...
const CHARACTER_CLASSES = [
  { id: 'pathfinder', name: 'Следопыт', statGroup: 'Стойкость', stats: ['Выносливость', 'Воля'], icon: Footprints, iconImg: pathfinderIconImg, color: '#e8633c', activities: ['running'] },
  { id: 'berserker', name: 'Берсеркер', statGroup: 'Мощь', stats: ['Сила', 'Упорство'], icon: Skull, iconImg: berserkerIconImg, color: '#c9a227', activities: ['strength_park', 'strength_gym'] },
  { id: 'battlemaster', name: 'Мастер битвы', statGroup: 'Рефлексы', stats: ['Силовая выносливость', 'Гибкость'], icon: Swords, iconImg: battlemasterIconImg, color: '#8a5cf6', activities: ['wrestling'] },
  { id: 'monk', name: 'Монах', statGroup: 'Самоконтроль', stats: ['Фокус', 'Дисциплина'], icon: Hand, iconImg: monkIconImg, color: '#4caf6d', activities: ['nutrition'] },
  { id: 'shaman', name: 'Шаман', statGroup: 'Жизненная сила', stats: ['Дух', 'ХП'], icon: Sparkle, iconImg: shamanIconImg, color: '#4f7cff', activities: ['sleep'] },
  { id: 'archmage', name: 'Архимаг', statGroup: 'Разум', stats: ['Интеллект', 'Мышление'], icon: Wand2, iconImg: archmageIconImg, color: '#d6558c', activities: ['reading'] },
];
    ...
    if (comboName) {
      ...
      if (clsA && clsB) {
        return {
          ...
          name: comboName,
          icon: clsA.icon,
          iconImg: clsA.iconImg,
          secondaryIcon: clsB.icon,
          secondaryIconImg: clsB.iconImg,
          color: clsA.color,
...
const ROAD_EVENT_PITY_DAYS = 2; // 2 календарных дня без события → гарантия на следующем логе
...
const SHOP_ITEMS = [
  ...
  { id: 'shop_common_shorts', slot: 'legs', rarity: 'common', name: 'Беговые шорты', icon: Footprints, price: 32, bonus: { activity: 'running', xpBonusPct: 3 } },
  { id: 'shop_common_squatbelt', slot: 'legs', rarity: 'common', name: 'Пояс для приседа', icon: Shield, price: 34, bonus: { activity: 'strength_gym', xpBonusPct: 3 } },
  { id: 'shop_common_warmupstick', slot: 'weapon', rarity: 'common', name: 'Палка для разминки', icon: Sword, price: 30, bonus: { activity: 'wrestling', xpBonusPct: 3 } },
  { id: 'shop_common_hydroflask', slot: 'weapon', rarity: 'common', name: 'Фляга бегуна', icon: Droplet, price: 28, bonus: { activity: 'running', xpBonusPct: 3 } },
  { id: 'shop_common_bookmark', slot: 'accessory', rarity: 'common', name: 'Закладка-скрепка', icon: BookOpen, price: 26, bonus: { activity: 'reading', xpBonusPct: 3 } },
  { id: 'shop_common_earplugs', slot: 'accessory', rarity: 'common', name: 'Беруши для сна', icon: Moon, price: 20, bonus: { activity: 'sleep', xpBonusPct: 3 } },
  { id: 'shop_common_apron', slot: 'body', rarity: 'common', name: 'Кухонный фартук', icon: Salad, price: 33, bonus: { activity: 'nutrition', xpBonusPct: 3 } },
  { id: 'shop_common_pouch', slot: 'accessory', rarity: 'common', name: 'Кошель туриста', icon: ShoppingBag, price: 40, bonus: { crystalPct: 3 } },
  ...
  { id: 'shop_rare_greaves', slot: 'legs', rarity: 'rare', name: 'Поножи скорохода', icon: Footprints, price: 120, bonus: { activity: 'running', xpBonusPct: 10 } },
  { id: 'shop_rare_kilt', slot: 'legs', rarity: 'rare', name: 'Килт силача', icon: Shield, price: 125, bonus: { activity: 'strength_park', xpBonusPct: 10 } },
  { id: 'shop_rare_seercane', slot: 'weapon', rarity: 'rare', name: 'Трость провидца', icon: Wand2, price: 128, bonus: { activity: 'reading', xpBonusPct: 10 } },
  { id: 'shop_rare_nunchaku', slot: 'weapon', rarity: 'rare', name: 'Нунчаки', icon: Swords, price: 130, bonus: { activity: 'wrestling', xpBonusPct: 10 } },
  // --- мульти-актив: +6% к двум активностям сразу ---
  { id: 'shop_rare_duobands', slot: 'hands', rarity: 'rare', name: 'Парные напульсники', icon: Hand, price: 132, bonus: { activities: ['strength_gym', 'strength_park'], xpBonusPct: 6 } },
  { id: 'shop_rare_travelcloak', slot: 'body', rarity: 'rare', name: 'Плащ двойного пути', icon: Shirt, price: 135, bonus: { activities: ['running', 'sleep'], xpBonusPct: 6 } },
  { id: 'shop_rare_sagebeads', slot: 'accessory', rarity: 'rare', name: 'Чётки мудреца', icon: Gem, price: 130, bonus: { activities: ['reading', 'nutrition'], xpBonusPct: 6 } },
  // --- кристалл-бонус ---
  { id: 'shop_rare_merchantring', slot: 'accessory', rarity: 'rare', name: 'Кольцо торговца', icon: Gem, price: 118, bonus: { crystalPct: 8 } },
...
const AVATAR_EMOJIS = [
  // ── КЛАССЫ ──────────────────────────────────────────
  { id: 'hood',           name: 'Капюшон следопыта',  group: 'Классы', svg: '🪖', img: pathfinderIconImg },
  { id: 'berserker_helm', name: 'Шлем берсерка',      group: 'Классы', svg: '⛑️', img: berserkerIconImg },
  { id: 'prayer_beads',   name: 'Чётки монаха',       group: 'Классы', svg: '📿', img: monkIconImg },
  { id: 'spirit_mask',    name: 'Маска шамана',       group: 'Классы', svg: '🎭', img: shamanIconImg },
  { id: 'archmage_hat',   name: 'Шляпа архимага',     group: 'Классы', svg: '🧙', img: archmageIconImg },
  { id: 'mace',           name: 'Булава мастера',     group: 'Классы', svg: '🏋️', img: battlemasterIconImg },
  // ── ОРУЖИЕ ──────────────────────────────────────────
  ...
  const statTotals = useMemo(() => {
    ...
    function baseXpFor(log) {
      ...
      if (log.activity === 'reading') {
        ...
        if (pages < 100) return 8;  // нормальная сессия
        return 11;                   // 100+ стр — серьёзная дисциплина / вершина (единый верхний брекет)
      }
                        ...
                        {groupAvatars.map(av => {
                          ...
                          return (
                            <button key={av.id} onClick={() => { onSelectAvatarEmoji(av.id); setShowAvatarPicker(false); }}
                              ...
                              {av.img
                                ? <img src={av.img} alt={av.name} width={30} height={30} style={{ borderRadius: 7, objectFit: 'cover', imageRendering: 'pixelated' }} />
...
function AchievementsView({ achievements, pentaResults, comboCounts, balanceCounts, secretAchievements, mythicAchievements, bestiaryAchievements, challengeAchievements, roadStoryAchievements, onOpenRecordsWall }) {
  const byId = {};
...
function CosmeticsSection({ purchasedFrameIds, equippedAvatarFrame, onBuyFrame, onEquipFrame, currencyBalance, ownedBackgrounds, onBuyBackground }) {
...
function ShopView({ currencyBalance, purchasedItemIds, shopItemAvailability, equippedShopItems, onPurchase, onEquip, onSell, onBuyConsumable, consumableLog, level, lockedClassId, purchasedFrameIds, equippedAvatarFrame, onBuyFrame, onEquipFrame, ownedBackgrounds, onBuyBackground }) {
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {(top3Classes || []).map((item) => (
              <button key={item.cls.id} onClick={() => onLockClass(item.cls.id)} style={{
                ...
                <ClassIcon cls={item.cls} size={20} color={item.cls.color} />
              ...
              .map(key => {
                ...
                return (
                  <button key={key} onClick={() => onChooseCombo(key, key)} style={{
                    ...
                    {otherCls && <ClassIcon cls={otherCls} size={18} color={otherCls.color} />}
                    <div style={{ flex: 1 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {CHARACTER_CLASSES.map(cls => (
          <div key={cls.id} style={{
            ...
            <ClassIcon cls={cls} size={20} color={cls.color} />
            <div style={{ flex: 1 }}>
...
function EquippedGear({ equippedShopItems, lockedClassId }) {
  ...
  return (
    <div style={styles.equippedGearWrap}>
      ...
      {(bonusEntries.length > 0 || equippedCrystalPct > 0) ? (
          ...
          <div style={styles.equippedBonusList}>
            ...
            {equippedCrystalPct > 0 && (
              <div style={styles.equippedBonusRow}>
                ...
                <span style={styles.equippedBonusValue}>+{equippedCrystalPct}%</span>
              </div>
            )}
            {allSlotsActive && (
...
function ClassCard({ currentClass, baseClass, isCombo, chosenPath, level }) {
  ...
  const baseIconCls = baseClass || currentClass;
  ...
  const comboSecondaryCls = isCombo ? { icon: currentClass.secondaryIcon, iconImg: currentClass.secondaryIconImg, name: currentClass.classB?.name } : null;
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: (isCombo || chosenPath) ? 14 : 0 }}>
        <div style={{
          ...
          <ClassIcon cls={baseIconCls} size={34} color={baseColor} style={{ filter: `drop-shadow(0 0 8px ${baseColor})` }} />
        </div>
        <div>
      ...
      {isCombo && (
        <div style={{
          ...
          <div style={{
            ...
            <ClassIcon cls={comboSecondaryCls} size={17} color={comboColor} style={{ filter: `drop-shadow(0 0 4px ${comboColor})` }} />
          </div>
          <div>
...
function renderAvatarContent(avatarEmoji, titleText, clsComponent, clsColor, size = 24) {
  if (avatarEmoji) {
    ...
    if (found) {
      if (found.img) return <img src={found.img} alt={found.name} width={size * 1.1} height={size * 1.1} style={{ borderRadius: Math.max(4, size * 0.25), objectFit: 'cover', imageRendering: 'pixelated' }} />;
      return <span style={{ fontSize: size * 1.1, lineHeight: 1 }}>{found.svg}</span>;
    }
  }
  const titleEmoji = getAvatarForTitle(titleText);
  if (titleEmoji) return <span style={{ fontSize: size, lineHeight: 1 }}>{titleEmoji}</span>;
  if (clsComponent) {
    return <ClassIcon cls={clsComponent.icon ? clsComponent : { icon: clsComponent }} size={size} color={clsColor} />;
  }
  ...
  function memberFromRow(row) {
    ...
    function memberBaseXp(l) {
      ...
      if (l.activity === 'reading') {
        ...
        if (p < 100) return 8;  // нормальная сессия
        return 11;               // 100+ стр — серьёзная дисциплина / вершина (единый верхний брекет)
      }
...
function ChallengesView({ challengeState, challengeProgress, activeChallengeBuff, challengeFailDebuff, challengeCooldownDays, level, onStart, secondChallengeSlotUnlocked, activeChallenge2, challengeProgress2, onStart2, onUnlockSecondSlot, currencyBalance }) {
