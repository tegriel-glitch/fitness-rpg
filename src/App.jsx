...
// ---------- AVATAR EMOJIS ----------
const AVATAR_EMOJIS = [
  { id: 'sword',      emoji: '⚔️',  name: 'Клинок' },
  { id: 'fire',       emoji: '🔥',  name: 'Пламя' },
  { id: 'dragon',     emoji: '🐉',  name: 'Дракон' },
  { id: 'fox',        emoji: '🦊',  name: 'Лис' },
  { id: 'skull',      emoji: '💀',  name: 'Череп' },
  { id: 'moon',       emoji: '🌙',  name: 'Луна' },
  { id: 'lightning',  emoji: '⚡',  name: 'Молния' },
  { id: 'eagle',      emoji: '🦅',  name: 'Орёл' },
  { id: 'wolf',       emoji: '🐺',  name: 'Волк' },
  { id: 'eye',        emoji: '👁️', name: 'Всевидящий' },
  { id: 'mountain',   emoji: '🏔️', name: 'Гора' },
  { id: 'wave',       emoji: '🌊',  name: 'Волна' },
  { id: 'leaf',       emoji: '🍃',  name: 'Лист' },
  { id: 'lion',       emoji: '🦁',  name: 'Лев' },
  { id: 'tiger',      emoji: '🐯',  name: 'Тигр' },
  { id: 'sakura',     emoji: '🌸',  name: 'Сакура' },
  { id: 'dagger',     emoji: '🗡️', name: 'Кинжал' },
  { id: 'amulet',     emoji: '🧿',  name: 'Амулет' },
  { id: 'diamond',    emoji: '💎',  name: 'Кристалл' },
  { id: 'butterfly',  emoji: '🦋',  name: 'Бабочка' },
  { id: 'tornado',    emoji: '🌪️', name: 'Вихрь' },
  { id: 'ice',        emoji: '🧊',  name: 'Лёд' },
  { id: 'comet',      emoji: '☄️', name: 'Комета' },
  { id: 'bear',       emoji: '🐻',  name: 'Медведь' },
  { id: 'snake',      emoji: '🐍',  name: 'Змей' },
  { id: 'crow',       emoji: '🐦‍⬛', name: 'Ворон' },
  { id: 'shield',     emoji: '🛡️', name: 'Щит' },
  { id: 'ghost',      emoji: '👻',  name: 'Призрак' },
  { id: 'bat',        emoji: '🦇',  name: 'Летучая мышь' },
  { id: 'storm',      emoji: '⛈️', name: 'Шторм' },
  { id: 'phoenix',    emoji: '🦜',  name: 'Феникс' },
  { id: 'panther',    emoji: '🐆',  name: 'Пантера' },
  { id: 'crown',      emoji: '👑',  name: 'Корона' },
  { id: 'bomb',       emoji: '💣',  name: 'Взрыв' },
  { id: 'vortex',     emoji: '🌀',  name: 'Вортекс' },
  { id: 'mushroom',   emoji: '🍄',  name: 'Гриб мудрости' },
];
...
function CharacterView({ statTotals, currentClass, equippedShopItems, purchasedItemIds, onEquip, earnedTitles, activeTitle, setActiveTitle, titleCooldownDays, levelState, lockedClassId, chosenPathId, achievementsEvaluated, purchasedFrameIds, equippedAvatarFrame, onEquipFrame, avatarEmoji, onSelectAvatarEmoji }) {
  ...
  return (
    <div style={{ animation: 'fadeUp 0.3s ease' }}>
      ...
      {/* AVATAR PICKER */}
      <div style={{ marginTop: 10, marginBottom: 10, background: '#1c1c22', border: '1.5px solid #28282f', borderRadius: 12, padding: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
          <span style={{ fontSize: 18 }}>🎭</span>
          <span style={{ fontWeight: 700, fontSize: 13, color: '#dcdce2' }}>Аватар</span>
          {avatarEmoji && (
            <span style={{ fontSize: 11, color: '#6a6a72' }}>
              — {AVATAR_EMOJIS.find(a => a.id === avatarEmoji)?.name}
            </span>
          )}
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 8 }}>
          {AVATAR_EMOJIS.map(av => {
            ...
            return (
              <button
                key={av.id}
                onClick={() => onSelectAvatarEmoji(av.id)}
                title={av.name}
                style={{
                  display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
                  gap: 2, padding: '8px 4px', borderRadius: 10, cursor: 'pointer',
                  background: isSelected ? '#3a5a1f' : '#13131a',
                  border: isSelected ? '1.5px solid #5a8a2f' : '1px solid #28282f',
                  transition: 'all 0.15s',
                }}
              >
                <span style={{ fontSize: 22, lineHeight: 1 }}>{av.emoji}</span>
                <span style={{ fontSize: 9, color: isSelected ? '#a8d86a' : '#5a5a62', textAlign: 'center', lineHeight: 1.2 }}>{av.name}</span>
              </button>
            );
          })}
        </div>
        {avatarEmoji && (
          <button
            onClick={() => onSelectAvatarEmoji(null)}
            style={{ marginTop: 10, fontSize: 11, color: '#6a6a72', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}
          >
            ✕ Сбросить аватар
          </button>
        )}
...
function renderAvatarContent(avatarEmoji, titleText, clsComponent, clsColor, size = 24) {
  if (avatarEmoji) {
    ...
    if (found) return <span style={{ fontSize: size * 1.1, lineHeight: 1 }}>{found.emoji}</span>;
  }
