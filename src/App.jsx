    ...
    <div style={styles.header}>
      <div style={styles.headerRow}>
        ...
        >
          ...
          <span style={{ fontSize: 10, color: '#5a5a72', fontWeight: 600 }}>{unlockedCount} ачивок</span>
        </button>
      </div>
...
function GuildView({ playerName, playerCharacterName, playerLevel, playerClass, playerPhysical, playerMental, playerTitleEntry, activeTitle, earnedTitles, raids, guildLikes, onLike, lockedClassId, chosenPathId, guildMembers, onRefreshGuild, onLogout, onTransferCrystals, onGiftItem, myPurchasedItemIds, myEquippedShopItems, myCurrencyBalance }) {
  ...
  function memberFromRow(row) {
    ...
    // Real achievements from ACHIEVEMENTS data
    const tierWeight = { platinum: 5, diamond: 4, gold: 3, silver: 2, bronze: 1, special: 3 };
    const memberAchievements = [];
    ACHIEVEMENTS.forEach(ach => {
      let currentValue = 0;
      if (ach.kind === 'total') {
        currentValue = memberLogs.filter(l => {
          if (Array.isArray(ach.activity)) return ach.activity.includes(l.activity);
          return l.activity === ach.activity;
        }).reduce((s, l) => {
          if (ach.unit === 'км') return s + (Number(l.distance) || 0);
          if (ach.unit === 'стр.') return s + (Number(l.pages) || 0);
          return s + 1;
        }, 0);
      } else if (ach.kind === 'streak') {
        currentValue = memberStreaks[ach.activity] || 0;
      } else if (ach.kind === 'books') {
        currentValue = (row.books || []).filter(b => b.finished).length;
      }
      let achievedTierIndex = -1;
      ach.tiers.forEach((t, idx) => { if (currentValue >= t.need) achievedTierIndex = idx; });
