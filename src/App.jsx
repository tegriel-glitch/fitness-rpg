...
export default function App() {
  ...
  useEffect(() => {
    stateRef.current = {
      ...
      guildLikes, lastLoginDate, loginStreak, morningRitualLog,
    };
  });
    ...
    <div style={{ border: `1.5px solid ${rc.border}`, background: rc.bg, borderRadius: 16, padding: 16, marginBottom: 16, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ background: '#1a1a22', border: '1px solid #2a2a32', borderRadius: 12, padding: 12, marginBottom: 12 }}>
          ...
          {isParticipant && (
            <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
              ...
              <button onClick={onCloseRaid} style={{ flex: 1, padding: '6px', borderRadius: 8, cursor: 'pointer', background: 'none', border: '1px solid #2a2a32', fontSize: 11, color: '#5a5a6a' }}>
                Отменить рейд
              </button>
            </div>
          )}
      ...
      {status === 'active' && (
        <>
          ...
          {isParticipant && (
            <button onClick={onCloseRaid} style={{ width: '100%', padding: '7px', borderRadius: 8, cursor: 'pointer', background: 'none', border: '1px solid #2a2a32', fontSize: 11, color: '#5a5a6a' }}>
              Отменить рейд
            </button>
          )}
        </>
  ...
  return (
    <div style={{ animation: 'fadeUp 0.3s ease' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {(CLASS_PATHS[lockedClassId] || []).map((path) => (
              <button key={path.id} onClick={() => onChoosePath(path.id)} style={{
                ...
                <div style={{ fontSize: 11, color: '#6a6a82', marginTop: 3 }}>{path.description}</div>
      ...
      {lockedClassId && chosenPathId && (() => {
        ...
        if (!path) return null;
        const SKILL_LEVELS = [10, 15, 20, 30, 40];
        return (
          <div style={{ marginTop: 12 }}>
            <SectionLabel text={`Навыки: ${path.name}`} />
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {path.skills.map((skill, i) => {
                const reqLvl = SKILL_LEVELS[i];
...
function GuildMemberDetail({ member, onClose, onNudge, onTransferCrystals, onGiftItem, myPurchasedItemIds, myEquippedShopItems, myCurrencyBalance }) {
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  {member.achievementNames.map((ach, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <span style={{ fontSize: 11 }}>{i === 0 ? '🥇' : i === 1 ? '🥈' : '🥉'}</span>
                      <span style={{ fontSize: 11.5, fontWeight: 700, color: i === 0 ? '#f5c84a' : i === 1 ? '#c0c0c0' : '#cd7f32' }}>
                        {ach}
                      </span>
                    </div>
                  ))}
...
function GuildView({ playerName, playerCharacterName, playerLevel, playerClass, playerPhysical, playerMental, playerTitleEntry, activeTitle, earnedTitles, raids, guildLikes, onLike, lockedClassId, chosenPathId, guildMembers, onRefreshGuild, onLogout, onTransferCrystals, onGiftItem, myPurchasedItemIds, myEquippedShopItems, myCurrencyBalance }) {
  ...
  function memberFromRow(row) {
    ...
    // Approximate per-log base XP (mirrors baseXpFor in the main app, without streak/shop/class bonuses)
    function approxBaseXp(l) {
      ...
      if (l.activity === 'calories') return 0; // calories don't feed stats/level, only crystals
      if (INTENSITY_ACTIVITIES.has(l.activity)) {
        return (INTENSITY_LEVELS[l.intensity] || INTENSITY_LEVELS.medium)?.xp || 10;
      }
      return 4; // nutrition / sleep flat baseline
    }

    // Build real stat totals from logs — each log's XP is added to every stat its activity trains
    // (matches how the main app distributes XP across def.stats in statTotals).
    ...
    const level = levelFromTotalXp(totalStatXp);
    const titleEntry = titleEntryForLevel(level);
    ...
    // Rough top-3 achievement badges for the card (approximate thresholds, not full ACHIEVEMENTS eval)
    const memberAchievements = [];
    if (totalKm >= 100) memberAchievements.push({ name: 'Марафонец', tier: 'gold' });
    else if (totalKm >= 42) memberAchievements.push({ name: 'Бегун', tier: 'silver' });
    else if (totalKm >= 10) memberAchievements.push({ name: 'Трусца', tier: 'bronze' });
    if (totalTrainings >= 30) memberAchievements.push({ name: 'Железный боец', tier: 'gold' });
    else if (totalTrainings >= 15) memberAchievements.push({ name: 'Тренированный', tier: 'silver' });
    else if (totalTrainings >= 5) memberAchievements.push({ name: 'Новичок зала', tier: 'bronze' });
    if (totalPages >= 500) memberAchievements.push({ name: 'Пожиратель знаний', tier: 'gold' });
    else if (totalPages >= 200) memberAchievements.push({ name: 'Читающий', tier: 'silver' });
    if ((activityCounts.nutrition || 0) >= 30) memberAchievements.push({ name: 'Гурман', tier: 'gold' });
    if ((activityCounts.sleep || 0) >= 21) memberAchievements.push({ name: 'Ночной Страж', tier: 'silver' });
    ...
    // Detect combo subclass using real stat totals (only when class isn't locked yet)
    const resolvedLive = resolveCharacterClass(memberStatTotals, level);
    const subclassName = (!row.locked_class_id && resolvedLive?.combo) ? resolvedLive.name : null;
