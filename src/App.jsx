...
export default function App() {
  ...
  const statTotals = useMemo(() => {
    ...
    return totals;
  }, [logs, healthState.poisonPenaltyEvents, healthState.xpBlockedByDate, comboResults.totalRewards, balanceResults.totalRewards, mythicAchievementsEvaluated, pentaResults.legendXpPerStat, pentaResults.godUnlocked, equippedShopItems, raids, raidArchive, books, lockedClassId, secretAchievementsEvaluated, morningRitualLog, challengeState, unomieDebuff]);
    ...
    Object.entries(raids).forEach(([bossId, raid]) => {
      ...
      if (lootData?.itemId) {
        setPurchasedItemIds((cur) => {
          ...
          // Persist immediately with the new item included
          setTimeout(() => scheduleSave(), 100);
          return updated;
        });
        setRaidShields(prev => { setTimeout(() => scheduleSave(), 200); return prev + 1; });
        ...
        // No class-specific loot — still give shield
        setRaidShields(prev => { setTimeout(() => scheduleSave(), 200); return prev + 1; });
        setToast({ text: '🏆 Победа в рейде! + 🛡️ Рейдовый щит', key: Date.now() });
        setTimeout(() => setToast(null), 4000);
      }
      setRaids((prev) => ({ ...prev, [bossId]: newRaid }));
...
function RaidsView({ raids, currentClass, characterName, onStartRaid, onJoinRaid, onSetReady, onLaunchRaid, onAddContribution, onResolve, onLeave, onCloseRaid, onRefresh, guildMembers, onCancelRaid, raidArchive, unomieDebuff }) {
  ...
  return (
    <div style={{ animation: 'fadeUp 0.3s ease' }}>
      ...
      {archiveDetail && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 80, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 16px' }}
          onClick={() => setArchiveDetail(null)}>
          <div style={{ background: '#1c1c22', border: '1.5px solid #2c2c34', borderRadius: 16, padding: 20, width: '100%', maxWidth: 380 }}
            onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
              <span style={{ fontSize: 36 }}>{archiveDetail.bossCreature}</span>
              <div>
                <div style={{ fontSize: 15, fontWeight: 800, color: archiveDetail.result === 'victory' ? '#7de87d' : '#e87d7d' }}>
                  {archiveDetail.result === 'victory' ? '🏆 Победа' : '💀 Провал'}
                </div>
                <div style={{ fontSize: 12, color: '#9a9aa2' }}>{archiveDetail.bossName} · {archiveDetail.date}</div>
              </div>
              <button onClick={() => setArchiveDetail(null)} style={{ marginLeft: 'auto', background: 'none', border: 'none', cursor: 'pointer', color: '#5a5a62', fontSize: 18 }}>✕</button>
            </div>
            <div style={{ fontSize: 11, color: '#6a6a72', marginBottom: 8 }}>Участники: {(archiveDetail.participants || []).join(', ')}</div>
            {archiveDetail.lootName && (
              <div style={{ fontSize: 12, color: RAID_RARITY_COLORS[archiveDetail.bossRarity]?.color || '#e0a868', background: '#1a2a1a', border: '1px solid #2a4a2a', borderRadius: 8, padding: '8px 12px', marginTop: 8 }}>
                🎁 Лут: {archiveDetail.lootName}
              </div>
            )}
          </div>
        </div>
        ...
        {chestOpen && (
          <div style={{ animation: 'fadeUp 0.2s ease', background: '#18181f', border: '1.5px solid #28282f', borderTop: 'none', borderRadius: '0 0 12px 12px', padding: 14 }}>
            ...
            <div style={{ marginTop: 14 }}>
              ...
              {SHOP_SLOTS.map((slot) => {
                const slotItems = ALL_ITEMS().filter((i) => i.slot === slot.key && purchasedItemIds.includes(i.id));
        ...
        {sorted.map((member) => {
          ...
          return (
            ...
            >
              ...
              {/* Active raid — compact single-line chip per raid */}
              {member.raidIds && member.raidIds.length > 0 && (
                  ...
                  {member.raidIds.map(rId => {
                    ...
                    return (
                      <div key={rId} style={{
                        ...
                        <span style={{ fontWeight: 700, color: rc }}>{boss.name}</span>
                        {stats && <span style={{ color: '#6a6a72' }}>{stats.pct}%</span>}
                      </div>
                    );
                  })}
                </div>
