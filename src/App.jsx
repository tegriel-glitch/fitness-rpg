...
async function dbLoadGuildMembers(excludeNickname) {
  const res = await sbFetch(`players?nickname=neq.${encodeURIComponent(excludeNickname)}&select=nickname,character_name,locked_class_id,chosen_path_id,logs,passive_logs,recovery_logs,raids,active_title,likes,purchased_item_ids,equipped_shop_items,unlocked_skill_levels,books`);
  if (!res.ok) return [];
...
export default function App() {
  ...
  const [consumableLog, setConsumableLog] = useState([]); // [{id, date, type}]
  ...
  async function selectNickname(nick) {
    ...
    try {
      ...
      if (row.consumable_log?.length) setConsumableLog(row.consumable_log);
      if (row.active_shield) setActiveShield(row.active_shield);
  ...
  // Save character name instantly to DB and localStorage on every change
  useEffect(() => {
  ...
  function resolveRaidDeadline(bossId) {
    setRaids((prev) => {
      ...
      // Grant loot item to inventory on victory
      if (victory) {
        const classId = effectiveClassId || 'pathfinder';
        ...
        if (lootData?.itemId) {
          ...
          setToast({ text: '🏆 Победа! Получен: ' + lootData.name, key: Date.now() });
          setTimeout(() => setToast(null), 4000);
        }
      }
  ...
  return (
    <div style={styles.app}>
      ...
      <div style={styles.content}>
        ...
        {tab === 'history' && <HistoryView logs={logs} passiveLogs={passiveLogs} recoveryLogs={recoveryLogs} onUndoLog={undoLog} onUndoPassive={undoPassiveLog} onUndoRecovery={undoRecoveryLog} />}
        {tab === 'rules' && <RulesView />}
  ...
  return (
    <div style={styles.header}>
      <div style={styles.headerRow}>
        ...
        <button
          ...
          style={{
            ...
            background: '#1e1c12', border: '1.5px solid #4a3a1f',
            borderRadius: 12, padding: '8px 12px', cursor: 'pointer',
            flexShrink: 0,
          }}
        >
          <Trophy size={22} color="#f0d272" />
          <span style={{ fontSize: 9, fontWeight: 700, color: '#d4af37', letterSpacing: 0.5 }}>АЧИВКИ</span>
          <span style={{ fontSize: 10, color: '#8a7a52', fontWeight: 600 }}>{unlockedCount}/{totalAchievements}</span>
        </button>
      </div>
      ...
      {activeShield && (() => {
        ...
        const now = new Date();
        const daysSince = Math.floor((now - shieldDate) / 86400000);
        if (daysSince < 1) return (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 10px', marginBottom: 6, background: '#0f2a1a', border: '1px solid #2a5a3a', borderRadius: 8 }}>
            <Shield size={12} color="#4ce0c0" />
            <span style={{ fontSize: 10, color: '#4ce0c0', fontWeight: 700 }}>Щит стрика активен</span>
          </div>
        );
        return null;
      })()}
...
function RulesView() {
  ...
  const sections = [
    {
      key: 'activities', title: '🏃 Активности', color: '#e0a868',
      content: `Каждый день ты можешь записать свои активности: бег, силовые (зал или парк), борьба, питание, сон, чтение, сжигание калорий.

За каждую запись ты получаешь опыт (XP), который распределяется по 12 характеристикам и поднимает твой общий уровень.

Бег — XP зависит от дистанции (км).
Силовые — XP зависит от интенсивности (лёгкая/средняя/высокая).
Борьба — XP зависит от интенсивности.
Питание — фиксированный XP за отметку правильного питания.
Сон — фиксированный XP + бонус за строгий режим (лёг до 23:00, встал до 09:00).
Чтение — XP зависит от количества страниц.
Пепел калорий — XP зависит от сожжённых ккал.`
    },
    {
      key: 'streaks', title: '🔥 Серии (стрики)', color: '#e8633c',
      content: `Серия — количество дней подряд, когда ты выполнял активность.

Серии дают бонусный XP и открывают достижения. Пропустил день — серия обнуляется.

Щит стрика (расходник в магазине) может спасти одну серию от прерывания. Покупается раз в 2 недели.`
    },
    {
      key: 'debuffs', title: '⚡ Дебаффы', color: '#e05f4a',
      content: `Дебаффы — негативные события, которые снижают здоровье:

Сильный стресс — снижает ментальное здоровье.
Недосып — снижает физическое здоровье.
Зажор / срыв диеты — снижает оба вида здоровья.

Дебаффы накапливаются: чем больше подряд, тем сильнее штраф. Отменить дебафф можно сразу после нажатия (кнопка «Отменить» в уведомлении).`
    },
    {
      key: 'recovery', title: '💚 Восстановление', color: '#4ce0c0',
      content: `Восстановление помогает вернуть потерянное здоровье.

Короткие (раз в день каждый тип): прогулка, растяжка, дыхательные упражнения, отдых.
Длительные (раз в день каждый тип): баня/сауна, массаж, медитация.

Каждый тип восстановления можно использовать один раз в день, но разные типы комбинируются.`
    },
    {
      key: 'health', title: '❤️ Система здоровья', color: '#e05f4a',
      content: `У персонажа два вида здоровья:

Физическое — зависит от активности, сна, дебаффов.
Ментальное — зависит от чтения, питания, стресса.

Если любой вид здоровья падает ниже 50%, рост опыта блокируется до восстановления.

Зелья из магазина восстанавливают ~10% здоровья (макс 3 в неделю).`
    },
    {
      key: 'classes', title: '⚔️ Классы и пути', color: '#9c6fe0',
      content: `Класс определяется твоей доминирующей активностью:

Следопыт — бег. Берсерк — силовые. Мастер битвы — борьба.
Монах — питание. Шаман — сон. Архимаг — чтение.

На 10 уровне класс фиксируется. После этого можно выбрать путь развития с уникальными пассивными навыками.

Если вторая активность достаточно прокачана (>15% от основной), открывается комбо-подкласс.`
    },
    {
      key: 'shop', title: '💎 Магазин и кристаллы', color: '#5b9bf0',
      content: `Кристаллы — валюта игры (~30 в день при активной игре).

В магазине 5 уровней редкости: обычные, редкие, эпические, легендарные, мифические.
Редкие открываются на 10 уровне, эпические на 20.
Легендарные и мифические получаются за особые достижения.

Экипировка даёт бонус к XP определённой активности.
Полный комплект (6 слотов) даёт +15% XP ко всему.

Расходники: щит стрика (раз в 2 недели), зелья здоровья и ясности (до 3 в неделю).
Предметы можно продать за 70% от цены.`
    },
    {
      key: 'raids', title: '⚔️ Рейды', color: '#e05f9c',
      content: `Рейды — совместные испытания для гильдии.

3 босса разной сложности: Вепрь (редкий), Лунный Орочи (эпический), Дракон (легендарный).

При старте рейда у группы есть дедлайн. Если не успели — штраф здоровью по редкости: редкий −20%, эпический −35%, легендарный −50%.

Победа даёт уникальные титулы и открывает лут.`
    },
    {
      key: 'achievements', title: '🏆 Достижения', color: '#f0d272',
      content: `Достижения разделены на категории:

Обычные — по каждой активности (бронза → серебро → золото → платина).
Тройной удар — комбо из 3 активностей за день.
Баланс — развитие нескольких направлений одновременно.
Пента-удар — 5 из 6 активностей за день.
Секретные — скрытые условия.
Мифические — самые сложные, привязаны к аниме-персонажам.

Каждый разблокированный тир даёт титул, который можно выбрать в разделе Персонаж. Титул можно менять раз в 3 дня.`
    },
    {
      key: 'guild', title: '🛡️ Гильдия', color: '#f0d272',
      content: `Гильдия — это ваша команда. Во вкладке «Гильдия» видны все игроки, их уровни, классы, здоровье и активные рейды.

Можно ставить «Уважуху» 👍 товарищам и «Пнуть» 👊 для мотивации — пинок придёт в виде уведомления.

Нажми на карточку товарища чтобы увидеть подробную статистику: стрики, тренировки, прогресс.`
    },
  ];
  ...
  return (
    <div style={{ animation: 'fadeUp 0.3s ease' }}>
      <div style={{ textAlign: 'center', marginBottom: 20 }}>
        ...
        <div style={{ fontSize: 11, color: '#6a6a72', marginTop: 4 }}>Нажми на раздел чтобы узнать подробности</div>
      </div>
        ...
        {sections.map(s => (
          <div key={s.key}>
            <button onClick={() => toggle(s.key)} style={{
              width: '100%', display: 'flex', alignItems: 'center', gap: 10,
              background: openSection === s.key ? s.color + '12' : '#1a1a22',
              border: '1.5px solid ' + (openSection === s.key ? s.color + '44' : '#26262e'),
              borderRadius: openSection === s.key ? '12px 12px 0 0' : 12,
              padding: '12px 14px', cursor: 'pointer', textAlign: 'left',
            }}>
              <span style={{ fontSize: 14, fontWeight: 800, color: s.color, flex: 1 }}>{s.title}</span>
              <ChevronDown size={14} color="#5a5a62" style={{ transform: openSection === s.key ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s', flexShrink: 0 }} />
            </button>
            {openSection === s.key && (
              <div style={{
                background: '#15151c', border: '1.5px solid ' + s.color + '44', borderTop: 'none',
                borderRadius: '0 0 12px 12px', padding: '14px 16px',
              }}>
                <div style={{ fontSize: 12, color: '#b0b0ba', lineHeight: 1.7, whiteSpace: 'pre-line' }}>
                  {s.content}
                </div>
              </div>
            )}
...
function ShopView({ currencyBalance, purchasedItemIds, shopItemAvailability, equippedShopItems, onPurchase, onEquip, onSell, onBuyConsumable, consumableLog, level, lockedClassId }) {
  const rarityOrder = ['common', 'rare', 'epic', 'legendary', 'mythic', 'consumables'];
  const RARITY_LEVEL_REQ = { common: 1, rare: 10, epic: 20, legendary: 1, mythic: 1 };
  const [activeRarity, setActiveRarity] = useState('common');
  const [slotFilter, setSlotFilter] = useState('all');

  const isConsumables = activeRarity === 'consumables';
  const activeItems = isConsumables ? [] : SHOP_ITEMS.filter((i) => i.rarity === activeRarity);
    ...
    <div style={{ animation: 'fadeUp 0.3s ease' }}>
      ...
      <div className="tabBarScroll" style={styles.shopRarityTabs}>
        {rarityOrder.map((rarityKey) => {
          const rTier = rarityKey === 'consumables' ? { color: '#4ce0c0', label: 'Расходники' } : RARITY_TIERS[rarityKey];
          const isActive = activeRarity === rarityKey;
          const count = rarityKey === 'consumables' ? CONSUMABLES.length : SHOP_ITEMS.filter((i) => i.rarity === rarityKey).length;
...
function CharacterView({ statTotals, currentClass, equippedShopItems, purchasedItemIds, onEquip, earnedTitles, activeTitle, setActiveTitle, titleCooldownDays, levelState, lockedClassId, chosenPathId, achievementsEvaluated }) {
...
function HistoryView({ logs, passiveLogs, recoveryLogs, onUndoLog, onUndoPassive, onUndoRecovery }) {
...
function GuildMemberDetail({ member, onClose, onNudge }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.9)', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}
      ...
      <div style={{ background: '#13131a', width: '100%', maxWidth: 420, margin: '24px auto', borderRadius: 16, padding: '0 0 20px', animation: 'fadeUp 0.25s ease' }}>
        ...
        {/* Header */}
        <div style={{ padding: '16px 18px 12px', display: 'flex', alignItems: 'flex-start', gap: 12, borderBottom: '1px solid #1e1e26' }}>
          <div style={{
            width: 52, height: 52, borderRadius: 14, flexShrink: 0,
            background: clsColor + '22', border: `2px solid ${clsColor}55`,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
          }}>
            {getAvatarForTitle(member.titleText)
              ? <span style={{ fontSize: 28 }}>{getAvatarForTitle(member.titleText)}</span>
              : member.cls ? <member.cls.icon size={26} color={clsColor} /> : <Skull size={26} color="#5a5a62" />
            }
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 18, fontWeight: 800, color: '#f0f0f4' }}>{(member.characterName || member.name || '').replace(/_/g, ' ')}</div>
            <div style={{ fontSize: 10, color: '#6a6a72', marginTop: 1 }}>@{(member.name || '').replace(/_/g, ' ')}</div>
            <div style={{ fontSize: 12, color: member.titleColor || '#e0a868', fontWeight: 700, marginTop: 3 }}>{member.titleText || 'Новичок'}</div>
          </div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#5a5a62', fontSize: 20, padding: '2px 4px' }}>✕</button>
        </div>

        {/* Level + Class strip */}
        <div style={{ padding: '12px 18px', display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 12, fontWeight: 800, color: '#f0f0f4', background: '#1e1e28', border: '1px solid #2a2a32', borderRadius: 6, padding: '3px 10px' }}>Ур. {member.level}</span>
          {member.cls && <span style={{ fontSize: 10.5, fontWeight: 700, color: clsColor, background: clsColor + '18', border: `1px solid ${clsColor}44`, borderRadius: 6, padding: '3px 8px' }}>{member.cls.name}</span>}
          {member.pathName && <span style={{ fontSize: 10.5, color: '#7a7a82', fontWeight: 600 }}>· {member.pathName}</span>}
        </div>
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                <div style={{
                  ...
                  background: (member.titleColor || clsColor) + '1f',
                  border: `2px solid ${(member.titleColor || clsColor)}55`,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                }}>
                  {getAvatarForTitle(member.titleText)
                    ? <span style={{ fontSize: 22 }}>{getAvatarForTitle(member.titleText)}</span>
