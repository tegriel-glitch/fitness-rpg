          ...
          {ritualXpBonus > 0 && (() => {
            ...
            return (
              <div style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '3px 8px', background: '#1a0f2a', border: '1px solid #3a2a5a', borderRadius: 8 }}>
                {todayRitual?.icon ? (
                  <img src={todayRitual.icon} alt={todayRitual.name} style={{ width: 22, height: 22, borderRadius: 6, objectFit: 'cover', flexShrink: 0, boxShadow: '0 0 6px 1px #c9a8f588' }} />
                ) : (
      ...
      <div style={styles.healthBarsWrap}>
        <div>
          ...
          {healthState.fatigueDebuffCount > 0 && (
            <DebuffIconStack src="/icons/debuffs/fatigue.webp" count={healthState.fatigueDebuffCount} alt="Усталость" />
          )}
        </div>
        <div>
          <HealthBar icon={Brain} label="" value={healthState.mental} color="#4f7cff" />
          {healthState.stressDebuffCount > 0 && (
            <DebuffIconStack src="/icons/debuffs/stress.webp" count={healthState.stressDebuffCount} alt="Стресс" />
          )}
    ...
    <div style={styles.poisonVialsRow}>
      {Array.from({ length: max }).map((_, idx) => {
        ...
        return (
          <img
            key={idx}
            src="/icons/debuffs/poison.webp"
            alt="Отравление"
            style={{
              width: 22, height: 22, borderRadius: 6,
              opacity: filled ? 1 : 0.25,
              filter: filled ? 'none' : 'grayscale(1)',
              boxShadow: filled ? '0 0 6px 1px #c9a8f588' : 'none',
...
// Stacking debuff icon row — each active stack renders as one small icon next to the last,
// since these debuffs stack (усталость/стресс). No text label, per design.
const DebuffIconStack = React.memo(function DebuffIconStack({ src, count, alt }) {
  return (
    <div style={{ display: 'flex', gap: 4, alignItems: 'center', marginTop: 3, flexWrap: 'wrap' }}>
      {Array.from({ length: count }).map((_, idx) => (
        <img
          ...
          style={{
            width: 22, height: 22, borderRadius: 6,
            boxShadow: '0 0 6px 1px #ff5c7a88',
    ...
    return (
      <div style={{ background: '#1c1c22', border: `1.5px solid ${catColor}33`, borderRadius: 12, padding: '10px 14px', marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 12, fontWeight: 800, color: '#f0f0f4', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{!def.icon && '🔥 '}{def.name}</div>
  ...
  function memberFromRow(row) {
    ...
    return {
      ...
      activeChallenge: (() => {
        ...
        const elapsed = Math.min(def.duration, Math.floor((today - startDate) / 86400000) + 1);
        const catDef = CHALLENGE_CATEGORIES.find(c => c.id === def.category);
        return { id: def.id, name: def.name, category: def.category, color: catDef?.color || '#e0a868', icon: def.icon || null, elapsed, duration: def.duration };
      })(),
      rawRow: row,
    };
  }
            ...
            >
              ...
              {member.activeChallenge && (
                <div style={{
                  ...
                  {member.activeChallenge.icon ? (
                    <img src={member.activeChallenge.icon} alt="" style={{ width: 18, height: 18, borderRadius: 5, objectFit: 'cover', flexShrink: 0 }} />
                  ) : (
    ...
    return (
      <div style={{ background: '#1c1c22', border: `1.5px solid ${catColor}44`, borderRadius: 14, padding: '12px 14px', marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
          <div style={{ width: 32, height: 32, borderRadius: 8, background: catColor + '22', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', flexShrink: 0 }}>
            {def.icon ? <img src={def.icon} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <CatIcon size={16} color={catColor} />}
          </div>
      ...
      <div>
        ...
        <div style={{ background: '#1c1c22', border: '1.5px solid ' + (catDef?.color || '#e0a868') + '44', borderRadius: 16, padding: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
            <div style={{ width: 44, height: 44, borderRadius: 11, background: (catDef?.color || '#e0a868') + '22', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', flexShrink: 0 }}>
