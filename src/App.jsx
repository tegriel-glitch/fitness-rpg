...
export default function App() {
  ...
  function scheduleSave(overrides = {}) {
    if (!selectedNickname || previewMode) return;
  ...
  function enterPreviewMode() {
    setPreviewMode(true);
    setSelectedNickname('__preview__');
    setCharacterName('Странник');
    const today = dateKey(new Date());
    const yesterday = dateKey(new Date(Date.now() - 86400000));
    const twoDaysAgo = dateKey(new Date(Date.now() - 86400000 * 2));
    setLogs([
      { id: 1, activity: 'running', date: today, distance: 5 },
      { id: 2, activity: 'strength_gym', date: today, intensity: 'medium' },
      { id: 3, activity: 'running', date: yesterday, distance: 3 },
      { id: 4, activity: 'sleep', date: yesterday, hours: 8, strict: false },
      { id: 5, activity: 'nutrition', date: yesterday },
    ]);
    setPassiveLogs([]);

    // Seed 2 mock guild members for preview
    setGuildMembers([
      {
        nickname: 'Рагнар',
        character_name: 'Рагнар Стальной',
        locked_class_id: 'berserker',
        chosen_path_id: 'berserker_destroyer',
        logs: [
          { id: 101, activity: 'running', date: today, distance: 7 },
          { id: 102, activity: 'strength_gym', date: today, intensity: 'high' },
          { id: 103, activity: 'wrestling', date: today, intensity: 'high' },
          { id: 104, activity: 'running', date: yesterday, distance: 4 },
          { id: 105, activity: 'strength_gym', date: yesterday, intensity: 'medium' },
          { id: 106, activity: 'nutrition', date: yesterday },
          { id: 107, activity: 'sleep', date: yesterday, hours: 7, strict: true },
          { id: 108, activity: 'running', date: twoDaysAgo, distance: 6 },
          { id: 109, activity: 'strength_park', date: twoDaysAgo, intensity: 'medium' },
          { id: 110, activity: 'reading', date: twoDaysAgo, pages: 30 },
          { id: 111, activity: 'calories', date: twoDaysAgo, kcal: 400 },
          { id: 112, activity: 'nutrition', date: twoDaysAgo },
          { id: 113, activity: 'sleep', date: twoDaysAgo, hours: 8, strict: true },
        ],
        passive_logs: [],
        recovery_logs: [],
        raids: { boar: { status: 'active', contributions: [] } },
        active_title: { text: 'Разрушитель', color: '#e8633c' },
        likes: {},
        purchased_item_ids: ['sword_1', 'helm_1'],
        equipped_shop_items: { weapon: 'sword_1', head: 'helm_1' },
        unlocked_skill_levels: [10, 15],
        books: [{ id: 1, title: '48 законов власти', finished: true, finishedDate: yesterday }],
      },
      {
        nickname: 'Серафим',
        character_name: 'Серафим Тихий',
        locked_class_id: 'monk',
        chosen_path_id: 'monk_ascetic',
        logs: [
          { id: 201, activity: 'reading', date: today, pages: 50 },
          { id: 202, activity: 'nutrition', date: today },
          { id: 203, activity: 'sleep', date: today, hours: 8, strict: true },
          { id: 204, activity: 'reading', date: yesterday, pages: 40 },
          { id: 205, activity: 'nutrition', date: yesterday },
          { id: 206, activity: 'sleep', date: yesterday, hours: 9, strict: true },
          { id: 207, activity: 'calories', date: yesterday, kcal: 600 },
          { id: 208, activity: 'running', date: twoDaysAgo, distance: 3 },
        ],
        passive_logs: [],
        recovery_logs: [],
        raids: {},
        active_title: { text: 'Аскет', color: '#4ce0c0' },
        likes: {},
        purchased_item_ids: [],
        equipped_shop_items: {},
        unlocked_skill_levels: [10],
        books: [
          { id: 1, title: 'Медитации', finished: true, finishedDate: twoDaysAgo },
          { id: 2, title: 'Путь воина', finished: false },
        ],
      },
    ]);
  }
  ...
  function changeTitle(newTitle) {
    ...
    if (lastTitleChangeDate) {
      ...
      if (daysSince < 3) return false; // cooldown active
    }
  ...
  useEffect(() => {
    if (!selectedNickname || previewMode || !characterName || characterName === 'Атлет') return;
            ...
            <>
              ...
              <button
                ...
                style={{
                  ...
                  background: 'none', border: '1px dashed #3a3a42',
                  borderRadius: 12, cursor: 'pointer',
                  fontSize: 12, color: '#5a5a6a', fontWeight: 600,
                  display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
                }}
              >
                👁 Режим предпросмотра (без БД)
              </button>
            </>
          )}
        </div>
  ...
  return (
    <div style={styles.app}>
      ...
      {previewMode && (
        <div style={{ background: '#1a1a10', border: '1px solid #4a4a1a', borderRadius: 8, padding: '6px 14px', margin: '0 0 8px', fontSize: 11, color: '#8a8a3a', textAlign: 'center' }}>
