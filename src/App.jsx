...
function AppInner() {
  ...
  const statTotals = useMemo(() => {
    ...
    function baseXpFor(log) {
      ...
      if (log.activity === 'reading') {
        ...
        return 11; // 200+ стр — единый верхний брекет (снижено 13→11 для баланса мага)
      }
  ...
  function rollRoadEvent(activityKey) {
    ...
    if (!pityTriggered && Math.random() > chance) {
      bumpLogsToday(today);
...
function AchievementsView({ achievements, pentaResults, comboCounts, balanceCounts, secretAchievements, mythicAchievements, bestiaryAchievements, challengeAchievements }) {
  const byId = {};
...
function ChallengesView({ challengeState, challengeProgress, activeChallengeBuff, challengeFailDebuff, challengeCooldownDays, level, onStart }) {
