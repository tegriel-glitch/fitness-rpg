...
function GuildView({ playerName, playerCharacterName, playerLevel, playerClass, playerPhysical, playerMental, playerTitleEntry, activeTitle, earnedTitles, raids, guildLikes, onLike, lockedClassId, chosenPathId, guildMembers, onRefreshGuild, onLogout, onTransferCrystals, onGiftItem, myPurchasedItemIds, myEquippedShopItems, myCurrencyBalance, myEquippedAvatarFrame }) {
  ...
  function memberFromRow(row) {
    ...
    const computedLevel = levelFromTotalXp(totalStatXp);
    const dbLevel = (row.current_level && row.current_level > 0) ? row.current_level : 0;
    // Берём максимум из БД и локального расчёта — оба могут быть неточными,
    // но реальный уровень не может быть НИЖЕ ни одного из них
    const level = Math.max(computedLevel, dbLevel, 1);

    // DEBUG: помогает найти проблему с отображением уровня в гильдии
    console.log(`[Guild] ${row.nickname}: db_level=${row.current_level}, computed=${computedLevel}, logs=${memberLogs.length}, totalXP=${Math.round(totalStatXp)}, final=${level}`);
