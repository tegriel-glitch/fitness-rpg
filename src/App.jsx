...
// Percent-based bounding boxes of the 6 equipment squares baked into the silhouette artwork
// (/public/character/equipment_silhouette.png, 326x512 source). Keeping these as % of the
// image's own box means they stay aligned regardless of screen width — key for mobile.
const SILHOUETTE_SLOT_BOXES = {
  head:      { left: 59.2, top: 6.6,  width: 24.5, height: 15.6 },
  body:      { left: 37.1, top: 23.6, width: 25.2, height: 15.6 },
  weapon:    { left: 0.3,  top: 36.3, width: 24.5, height: 15.6 },
  hands:     { left: 74.5, top: 36.3, width: 24.5, height: 15.6 },
  legs:      { left: 37.4, top: 65.6, width: 24.5, height: 16.0 },
  accessory: { left: 1.8,  top: 65.8, width: 25.5, height: 16.2 },
};

const EquippedGear = React.memo(function EquippedGear({ equippedShopItems, lockedClassId, onSell, onSlotTap }) {
      ...
      <React.Fragment key={slot.key}>
        ...
        >
          {item
            ? (item.image
                ? <img src={item.image} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                : <item.icon size={22} color={rarity.color} />)
            : <Plus size={16} color="#5a5a62" />}
          {item && !active && (
            <div style={{
              position: 'absolute', top: 2, right: 2,
              background: '#3a1a1a', border: '1px solid #7a3a3a', borderRadius: 5,
              padding: '0px 3px', fontSize: 7, color: '#e08a8a', fontWeight: 700,
            }}>
              !
            </div>
          )}
        ...
        {item && (
          <div
            ...
            style={{
              position: 'absolute',
              left: box.left + '%', top: (box.top + box.height + 1) + '%', width: box.width + '%',
              display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1,
              cursor: 'pointer', opacity: displayOpacity, pointerEvents: 'auto',
            }}
          >
            <span style={{
              fontSize: 7.5, fontWeight: 700, color: active ? '#dcdce2' : '#8a8a92', textAlign: 'center',
            ...
            {item.bonus && (item.bonus.xpBonusPct || item.bonus.crystalPct) && (
              <span style={{ fontSize: 6.5, lineHeight: 1.1, textAlign: 'center' }}>
                {renderBonusColored(item.bonus)}
              </span>
            )}
          </div>
  ...
  return (
    <div style={styles.equippedGearWrap}>
      <div style={{ position: 'relative', width: '100%', maxWidth: 300, margin: '0 auto', paddingTop: `${(512 / 326) * 100}%` }}>
        <img
          src="/character/equipment_silhouette.png"
          alt=""
