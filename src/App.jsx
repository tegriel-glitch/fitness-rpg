...
function AppInner() {
  ...
  useEffect(() => {
    Object.entries(raids).forEach(([bossId, raid]) => {
      ...
      const trophyAlreadyReceived = (raidTrophiesReceived || []).includes(bossId);
  ...
  }, [raids, characterName, effectiveClassId, raidSetProgress, raidTrophiesReceived]);
...
const BossLoreModal = React.memo(function BossLoreModal({ boss, onClose, currentClass, raidSetProgress, raidTrophiesReceived }) {
  ...
  const trophyReceived = (raidTrophiesReceived || []).includes(boss.id);
...
const RaidBossCard = React.memo(function RaidBossCard({ boss, raid, currentClass, characterName, onStart, onJoin, onReady, onLaunch, onAddContribution, onResolve, onLeave, onCloseRaid, guildMembers, onCancelRaid, raidSetProgress, raidTrophiesReceived }) {
  ...
  const setTrophyReceived = (raidTrophiesReceived || []).includes(boss.id);
...
const CharacterView = React.memo(function CharacterView({ statTotals, currentClass, equippedShopItems, purchasedItemIds, onEquip, onSell, earnedTitles, activeTitle, setActiveTitle, titleCooldownDays, levelState, lockedClassId, chosenPathId, achievementsEvaluated, purchasedFrameIds, equippedAvatarFrame, onEquipFrame, avatarEmoji, onSelectAvatarEmoji, shopShields, lastShopShieldUse, activeShield, onUseShopShield, onOpenRecordsWall, ownedBackgrounds, activeBackground, onBuyBackground, onEquipBackground, currencyBalance, polishedItemIds, itemBonusOverrides, onPolishItem, onReforgeItem, getEffectiveBonus, raidSetProgress, raidTrophiesReceived }) {
    ...
    <div style={{ animation: 'fadeUp 0.3s ease' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {REPEATABLE_SET_BOSS_IDS.map((bossId) => {
          ...
          const trophyReceived = (raidTrophiesReceived || []).includes(bossId);
          const rc = RAID_RARITY_COLORS[boss.rarity] || RAID_RARITY_COLORS.rare;
