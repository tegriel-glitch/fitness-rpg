...
export default function App() {
  ...
  const challengeProgress = useMemo(() => {
    ...
    const daysRemaining = def.duration - elapsed;

    // For count-based challenges (N activities in M days): fail only when it's
    // mathematically impossible to reach the goal with remaining days.
    // For daily challenges (do X every day): fail when misses exceed allowedMisses.
    const isFailed = def.countBased
      ? completedSoFar + daysRemaining < (def.countGoal || 1)
    ...
    return { def, dayResults, elapsed, missedSoFar, maxAllowed, isFailed, isComplete, perfectDays, completedSoFar };
...
function ChallengesView({ challengeState, challengeProgress, activeChallengeBuff, challengeFailDebuff, challengeCooldownDays, challengeAchievements, level, onStart }) {
  ...
  const activeCompact = challengeProgress && challengeState.active ? (() => {
    const { def, dayResults, elapsed, missedSoFar, maxAllowed } = challengeProgress;
    ...
    return (
      <div style={{ background: '#1c1c22', border: `1.5px solid ${catColor}44`, borderRadius: 14, padding: '12px 14px', marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'space-between', fontSize: 10 }}>
          {def.countBased ? (
            <>
              <span style={{ color: '#5adf5a' }}>✓ {successDays}/{def.countGoal} тренировок</span>
        ...
        <div style={{ background: '#1c1c22', border: '1.5px solid ' + (catDef?.color || '#e0a868') + '44', borderRadius: 16, padding: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 11.5, color: '#8a8a92' }}>{def.duration} дней · {def.countBased ? `цель: ${def.countGoal} тренировок` : `допуск: ${def.allowedMisses} пропуск(ов)`}</div>
            </div>
          </div>
