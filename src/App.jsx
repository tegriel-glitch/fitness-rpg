...
// (/public/character/equipment_silhouette.png, 326x512 source). Measured directly off the
// approved artwork via contour detection so slots line up exactly with the drawn frames.
const SILHOUETTE_SLOT_BOXES = {
  head:      { left: 60.1, top: 7.2,  width: 23.0, height: 14.6 },
  body:      { left: 38.0, top: 24.2, width: 23.6, height: 20.9 },
  weapon:    { left: 1.2,  top: 36.9, width: 23.0, height: 14.6 },
  hands:     { left: 75.5, top: 36.9, width: 23.0, height: 14.6 },
  legs:      { left: 38.3, top: 66.2, width: 23.0, height: 15.0 },
  accessory: { left: 1.5,  top: 66.0, width: 25.2, height: 15.6 },
};
...
const EquippedGear = React.memo(function EquippedGear({ equippedShopItems, lockedClassId, onSell, onSlotTap }) {
    ...
    return (
      <React.Fragment key={slot.key}>
        ...
        {item && (
          ...
          >
            <span style={{
              fontSize: 8.5, fontWeight: 700, color: active ? '#dcdce2' : '#8a8a92', textAlign: 'center',
            ...
            {item.bonus && (item.bonus.xpBonusPct || item.bonus.crystalPct) && (
              <span style={{ fontSize: 7.5, lineHeight: 1.1, textAlign: 'center' }}>
                {renderBonusColored(item.bonus)}
              </span>
            )}
          </div>
  ...
  return (
    <div style={styles.equippedGearWrap}>
      <div style={{ position: 'relative', width: '100%', paddingTop: `${(512 / 326) * 100}%` }}>
        <img
