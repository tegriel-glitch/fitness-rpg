...
const RaidBossCard = React.memo(function RaidBossCard({ boss, raid, currentClass, characterName, onStart, onJoin, onReady, onLaunch, onAddContribution, onResolve, onLeave, onCloseRaid, guildMembers, onCancelRaid, raidSetProgress, raidTrophiesReceived }) {
  ...
  return (
    <div style={{ border: `1.5px solid ${rc.border}`, background: rc.bg, borderRadius: 16, marginBottom: 10, position: 'relative', overflow: 'hidden' }}>
      ...
      {/* Сет-лут с этого босса — сама иконки предметов, не только бейджик в шапке */}
      {isSetBoss && setItemsForCard.length > 0 && (
        <div style={{ background: '#15151a', border: `1px solid ${rc.color}33`, borderRadius: 12, padding: 12, marginBottom: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: rc.color, flex: 1 }}>
              {!setTrophyReceived ? '🏆 Трофей за первую победу' : '🎁 Лут сета'}
            </span>
            {setTrophyReceived && (
              <span style={{
                ...
                {setComplete ? `✅ ${setClaimedForCard.length}/${setItemsForCard.length}` : `${setClaimedForCard.length}/${setItemsForCard.length}`}
              </span>
            )}
          </div>
          {!setTrophyReceived ? (
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {setItemsForCard.map((it) => {
                const has = setClaimedForCard.includes(it.itemId);
                return (
