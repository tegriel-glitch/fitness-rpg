...
async function dbLoadMessages(nickname) {
  try {
    const res = await sbFetch(`messages?to_nickname=eq.${encodeURIComponent(nickname)}&order=created_at.desc&limit=50`);
    if (!res.ok) return [];
    return await res.json();
  } catch (_) { return []; }
}
...
const COMBO_ACHIEVEMENTS = [
  ...
  },
  {
    ...
    requires: ['walking_15k', 'strength', 'nutrition'],
    description: '15к+ шагов + силовая + питание в один день',
    rewards: { 'Упорство': 4, 'Выносливость': 4 },
  },
  {
    id: 'combo_path_of_sage',
    title: 'Путь мудреца',
    requires: ['walking_15k', 'reading', 'sleep'],
    description: '15к+ шагов + чтение + здоровый сон в один день',
...
const ROAD_EVENT_CHANCE_BY_LEVEL = [
  { maxLevel: 5,  chance: 0.30, types: ['find'] },
  { maxLevel: 14, chance: 0.25, types: ['find', 'encounter'] },
  { maxLevel: 30, chance: 0.20, types: ['find', 'encounter', 'rare'] },
  { maxLevel: 99, chance: 0.20, types: ['find', 'encounter', 'rare'] },
];
...
const SHOP_ITEMS = [
  ...
  { id: 'shop_epic_boots', slot: 'legs', rarity: 'epic', name: 'Сапоги странника', icon: Footprints, price: 200, bonus: { activity: 'walking', xpBonusPct: 20 } },
  ...
  { id: 'shop_common_pilgrim_boots', slot: 'legs', rarity: 'common', name: 'Сапоги ходока', icon: Footprints, price: 35, bonus: { activity: 'walking', xpBonusPct: 10 } },
...
export default function App() {
    ...
    try {
      ...
      if (hour >= 18 && !localStorage.getItem(warnKey) && row.logs) {
        ...
        ['running', 'nutrition', 'sleep', 'reading'].forEach(act => {
          const streak = computeCurrentStreak(playerLogs.filter(l => l.activity === act).map(l => l.date));
          if (streak < 3) return;
  ...
  const zeroActivityDays = useMemo(() => {
    ...
    return computeZeroActivityDays(logs.map((l) => l.date), stepsDatesAbove3k);
  }, [logs, logsByActivity]);
  ...
  const achievementsEvaluated = useMemo(() => ACHIEVEMENTS.map((a) => ({ ...a, ...evaluateAchievement(a) })), [streaksByActivity, maxRunDistance, totalRunDistance, weeklyStrengthCount, styleCounts, totalCountByActivity, calBestDay, calTotal, calStreak300, calStreak500, totalReadingPages, booksFinishedCount, stepsBestDay, stepsTotal, stepsLogs]);
    ...
    combined.forEach((entry) => {
      ...
      if (entry._kind === 'activity') {
        ...
        if (entry.activity === 'calories' && entry.kcal) {
          ...
          if (kcalByDate[entry.date] >= 1500 && !calorieFatigueAppliedDates.has(entry.date)) {
  ...
  const statTotals = useMemo(() => {
    ...
    function baseXpFor(log) {
      ...
      if (log.activity === 'walking') {
        ...
        // XP = шаги / 1000 × 3, с затуханием выше 20000
        const normalSteps = Math.min(steps, 20000);
        const overSteps = Math.max(0, steps - 20000);
        return Math.round(normalSteps / 1000 * 3 + overSteps / 1000 * 3 * 0.2);
      }
    ...
    sortedAsc.forEach((log) => {
      ...
      let xpForThisLog = blocked ? 0 : base * multiplier * classBonus * shopBonus * weeklyTrainingBonus * ritualBonus * challengeBuffMult * challengeDebuffMult * roadBuffMult;
  ...
  // Steps crystals: threshold-based per day (5k→5, 8k→10, 12k+→15, cap 15)
  const stepsCrystals = useMemo(() => {
    ...
    return Object.values(byDate).reduce((sum, total) => {
      if (total >= 12000) return sum + 10;
      if (total >= 8000)  return sum + 7;
      if (total >= 5000)  return sum + 3;
      return sum;
    }, 0);
  }, [logsByActivity]);
  ...
  function submitLog() {
    ...
    // Roll for road story event (delayed to show after toast)
    const capturedActivity = selectedActivity;
    setTimeout(() => triggerRoadEvent(capturedActivity), 1800);
  ...
  function useRaidShield() {
    if (raidShields <= 0) {
      setToast({ text: 'Нет рейдовых щитов', key: Date.now() });
      setTimeout(() => setToast(null), 2600);
      return;
    }
    const today = dateKey(new Date());
    if (activeShield && activeShield.date === today) {
      setToast({ text: 'Щит уже активен сегодня', key: Date.now() });
      setTimeout(() => setToast(null), 2600);
      return;
    }
    setRaidShields(prev => prev - 1);
    setActiveShield({ date: today, type: 'raid' });
    setToast({ text: '🛡️ Рейдовый щит активирован! Все стрики защищены на сутки.', key: Date.now() });
    setTimeout(() => setToast(null), 3500);
  }
  ...
  useEffect(() => {
    Object.entries(raids).forEach(([bossId, raid]) => {
      ...
      if (lootData?.itemId) {
        ...
        setRaidShields(prev => prev + 1);
        setToast({ text: '🏆 Победа в рейде! Получен: ' + lootData.name + ' + 🛡️ Рейдовый щит', key: Date.now() });
        setTimeout(() => setToast(null), 4000);
        if (selectedNickname) dbSendMessage('__system__', selectedNickname, `Победа в рейде! Получен: ${lootData.name} и 🛡️ Рейдовый щит!`, 'Внутренний голос', 'raid');
      }
      setRaids((prev) => ({ ...prev, [bossId]: newRaid }));
  ...
  return (
    <div style={styles.header}>
      <div style={styles.headerRow}>
        ...
        {/* Raid shields */}
        {raidShields > 0 && (
          <button onClick={onUseRaidShield} style={{
            display: 'flex', alignItems: 'center', gap: 3, fontSize: 11, color: '#4ce0c0',
            background: '#0f2a1a', border: '1px solid #2a5a3a', borderRadius: 12, padding: '4px 8px',
            cursor: 'pointer', flexShrink: 0,
          }}>
            <Shield size={11} color="#4ce0c0" />
            <span>{raidShields}</span>
          </button>
        )}
        <button
...
const BERSERKER_ATTACKS = {
  quantitative: { low: ['{name} с рёвом бросается на {boss}, но толстая шкура не пропускает удар глубоко.', '{name} замахивается боевым топором — удар скользит по броне монстра.'], mid: ['{name} входит в ярость! Кровавый топор рассекает плоть {boss}, выбивая сноп искр!', '{name} оглушает {boss} серией бешеных ударов, земля трясётся под ногами!'], high: ['{name} теряет рассудок от ярости! Мощнейший удар топором РАЗРУБАЕТ {boss} до кости! {value} урона!', '{name} превращается в берсерка древних! Кровавый вихрь ударов разрывает {boss} в клочья!'] },
  endurance: { early: ['{name} стискивает зубы и держит боевую стойку. {boss} пытается сломить волю берсерка.'], mid: ['{name} рычит от напряжения — {boss} давит на разум, но ярость берсерка не угасает!'], late: ['{name} ревёт как обезумевший зверь! Никакая магия {boss} не сломит его железную волю!'] },
  strength_count: { 1: ['{name} пробует силу на {boss} — первый удар оставляет лишь царапину.'], 2: ['{name} наращивает темп! Топор врезается глубже — {boss} отступает!'], 3: ['{name} в боевом трансе! Удары сыпятся как град — {boss} теряет равновесие!'], 4: ['{name} НАНОСИТ СОКРУШИТЕЛЬНЫЙ УДАР! {boss} падает на колени, земля раскалывается!'] },
};
const PATHFINDER_ATTACKS = {
  quantitative: { low: ['{name} выпускает стрелу — она вонзается в шкуру {boss}, но неглубоко.', '{name} стремительно обегает {boss} с фланга и наносит удар на ходу.'], mid: ['{name} метким выстрелом попадает в слабое место {boss}! Стрела проходит насквозь!', '{name} набирает невероятную скорость и врезается в {boss} как ветер!'], high: ['{name} метким выстрелом отравленной стрелы попадает в глаз {boss}! Монстр ослеплён! {value} урона!', '{name} выпускает залп стрел — каждая находит своё слабое место! {boss} пронзён десятком стрел!'] },
  endurance: { early: ['{name} занимает позицию на расстоянии, следя за каждым движением {boss}.'], mid: ['{name} уворачивается от атак {boss} с невероятной ловкостью — ни одна не достигает цели!'], late: ['{name} стал неуловимым призраком! {boss} бессильно хлещет воздух — следопыт всегда на шаг впереди!'] },
  strength_count: { 1: ['{name} осторожно пускает разведывательную стрелу в {boss}.'], 2: ['{name} находит уязвимое место — стрелы летят точнее!'], 3: ['{name} в идеальном ритме! Стрелы свистят одна за другой — {boss} не может укрыться!'], 4: ['{name} ВЫПУСКАЕТ СТРЕЛУ ВОЗМЕЗДИЯ! Она проходит сквозь {boss} насквозь!'] },
};
const MONK_ATTACKS = {
  quantitative: { low: ['{name} сосредотачивает ки и усиливает защиту союзников. Бафф: +сопротивление!', '{name} входит в медитацию и окутывает команду аурой спокойствия.'], mid: ['{name} активирует Стальную Стойку! Вся команда получает +30% к защите от атак {boss}!', '{name} направляет поток ки через всех союзников — их раны затягиваются!'], high: ['{name} раскрывает Врата Силы! Мощнейший бафф пронизывает всю команду — {boss} чувствует нарастающую угрозу!', '{name} входит в состояние Абсолютного Фокуса! Каждый удар команды наносит двойной урон!'] },
  endurance: { early: ['{name} создаёт ментальный щит вокруг команды. {boss} не может пробить защиту разума.'], mid: ['{name} укрепляет дух союзников! «Ваш разум — ваша крепость! Не сдавайтесь!»'], late: ['{name} достигает просветления! Аура монаха делает всю команду неуязвимой к ментальным атакам {boss}!'] },
  strength_count: { 1: ['{name} благословляет команду перед боем — лёгкий бафф к атаке.'], 2: ['{name} направляет ки в удар союзника — следующая атака бьёт вдвое сильнее!'], 3: ['{name} активирует Ки-Взрыв! Волна энергии оглушает {boss} и усиливает всю команду!'], 4: ['{name} ОТКРЫВАЕТ ВСЕ ВОСЕМЬ ВРАТ! Команда сражается с силой десятерых!'] },
};
const SHAMAN_ATTACKS = {
  quantitative: { low: ['{name} призывает духов-волков — они кусают {boss}, отвлекая внимание.', '{name} ставит тотем исцеления — команда восстанавливает силы.'], mid: ['{name} призывает духов волков, атакующих {boss} — они раздирают его до крови!', '{name} вызывает грозовой тотем! Молния бьёт в {boss}, парализуя монстра!'], high: ['{name} призывает ВЕЛИКОГО ДУХА! Призрачный медведь обрушивается на {boss}! {value} урона!', '{name} входит в транс предков — духи сотен шаманов атакуют {boss} одновременно!'] },
  endurance: { early: ['{name} окуривает лагерь целебным дымом. Команда чувствует прилив сил.'], mid: ['{name} вызывает духов-хранителей — они окружают команду, отгоняя тёмные чары {boss}!'], late: ['{name} достигает связи с миром духов! Целительная аура полностью восстанавливает команду — {boss} бессилен!'] },
  strength_count: { 1: ['{name} ставит тотем подготовки — духи присматриваются к {boss}.'], 2: ['{name} активирует тотем ярости — духи начинают атаковать {boss}!'], 3: ['{name} входит в боевой транс! Тотемы полыхают — духи рвут {boss} на части!'], 4: ['{name} ПРИЗЫВАЕТ ДУХА-РАЗРУШИТЕЛЯ! Колоссальный призрак сокрушает {boss}!'] },
};
const BATTLEMASTER_ATTACKS = {
  quantitative: { low: ['{name} сближается с {boss} и проводит серию ударов — монстр пошатнулся.', '{name} перехватывает атаку {boss} и контратакует в ближнем бою.'], mid: ['{name} проводит мощный комбо-удар! Кулаки мелькают как молнии — {boss} отступает!', '{name} захватывает {boss} в клинч и наносит серию разрушительных апперкотов!'], high: ['{name} входит в идеальный боевой ритм! Каждый удар — критический! {boss} не может защититься! {value} урона!', '{name} проводит ФИНИШЕР! Мощнейший удар пробивает защиту {boss} насквозь!'] },
  endurance: { early: ['{name} принимает боевую стойку, готовый к длительному бою с {boss}.'], mid: ['{name} держит оборону стальным захватом! {boss} не может пробить защиту мастера!'], late: ['{name} стоит как скала! {boss} измотан попытками сломить несокрушимого мастера битвы!'] },
  strength_count: { 1: ['{name} делает разведку боем — первый удар оценивает защиту {boss}.'], 2: ['{name} нащупал слабое место! Серия точных ударов ломает защиту {boss}!'], 3: ['{name} в режиме полного контроля! Каждое движение — идеальная техника!'], 4: ['{name} РАСКРЫВАЕТ ЗАПРЕТНУЮ ТЕХНИКУ! Удар проходит сквозь любую защиту — {boss} ПОВЕРЖЕН!'] },
};
const ARCHMAGE_ATTACKS = {
  quantitative: { low: ['{name} произносит заклинание — магический снаряд ударяет в {boss}, оставляя ожог.', '{name} открывает книгу заклятий и наносит лёгкий магический удар по {boss}.'], mid: ['{name} читает древнее заклинание! Огненный шар врезается в {boss} — земля плавится!', '{name} призывает ледяную бурю — {boss} скован морозом и не может двигаться!'], high: ['{name} произносит ЗАКЛИНАНИЕ ВЫСШЕГО ПОРЯДКА! Поток чистой магии испепеляет {boss}! {value} урона!', '{name} призывает метеорный дождь! Небеса обрушиваются на {boss} — взрывы сотрясают землю!'] },
  endurance: { early: ['{name} плетёт магический барьер. {boss} не может проникнуть сквозь чары.'], mid: ['{name} усиливает защитные чары! Ментальные атаки {boss} разбиваются о щит архимага!'], late: ['{name} создаёт АБСОЛЮТНЫЙ БАРЬЕР! {boss} истощает свою силу впустую — магия архимага непробиваема!'] },
  strength_count: { 1: ['{name} изучает слабости {boss} магическим зрением.'], 2: ['{name} наносит точечный магический удар в слабое место {boss}!'], 3: ['{name} складывает многослойное заклинание — магия пронизывает {boss} насквозь!'], 4: ['{name} ПРИЗЫВАЕТ МЕТЕОР УНИЧТОЖЕНИЯ! Магия архимага стирает {boss} с лица земли!'] },
};
...
const DEFAULT_ATTACKS = {
  quantitative: { low: ['{name} атакует {boss} — урон невелик, но удар засчитан.'], mid: ['{name} наносит мощный удар по {boss}! Монстр отступает!'], high: ['{name} обрушивает СОКРУШИТЕЛЬНЫЙ удар на {boss}! {value} урона!'] },
  endurance: { early: ['{name} держится стойко против {boss}.'], mid: ['{name} выдерживает натиск {boss} — воля не сломлена!'], late: ['{name} стоит до конца! {boss} не может сломить героя!'] },
  strength_count: { 1: ['{name} наносит первый осторожный удар по {boss}.'], 2: ['{name} усиливает натиск — {boss} чувствует давление!'], 3: ['{name} входит в раж! Удары сыпятся один за другим!'], 4: ['{name} НАНОСИТ ФИНАЛЬНЫЙ УДАР! {boss} содрогается от мощи!'] },
};
...
function RaidsView({ raids, currentClass, characterName, onStartRaid, onJoinRaid, onSetReady, onAddContribution, onResolve, onLeave, onCloseRaid, onRefresh, guildMembers }) {
...
function RaidBossCard({ boss, raid, currentClass, characterName, onStart, onJoin, onReady, onAddContribution, onResolve, onLeave, onCloseRaid, guildMembers }) {
  ...
  function handleContrib() {
    ...
    if (overrideMode) {
      if (contribValue === '' || isNaN(v) || v < 0) return; // allow 0 in override
    } else {
      if (!v || v <= 0) return;
    }
...
function MailModal({ messages, onClose, onMarkRead, onMarkAllRead, onDeleteMessages, onSendReply, playerNickname }) {
  ...
  const typeIcon = (msg) => { if (!msg.system_type) return '👤'; const icons = { nudge: '👊', achievement: '🏆', raid: '🏰', raid_hit: '⚔️', streak_warning: '🔥', level_up: '⚡', spirit_challenge: '🧿' }; return icons[msg.system_type] || '🔮'; };
  const typeBorderColor = (msg) => { if (!msg.system_type) return '#3a5a3a'; const colors = { nudge: '#8a5cf6', achievement: '#f0d272', raid: '#e8633c', raid_hit: '#e8633c', streak_warning: '#e05f4a', level_up: '#4ce0c0', spirit_challenge: '#4f7cff' }; return colors[msg.system_type] || '#4f7cff'; };
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.92)', overflowY: 'auto' }} onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
          ...
          return (
            ...
            >
              ...
              {member.raidIds && member.raidIds.length > 0 && member.raidIds.map(rId => {
                ...
                const deadlineD = startD ? new Date(startD.getTime() + boss.durationDays * 86400000) : null;
                const daysLeft = startD
                  ? Math.max(0, boss.durationDays - Math.floor((Date.now() - startD.getTime()) / 86400000))
                  : boss.durationDays;
                let countdownGuild = '';
                if (deadlineD && deadlineD > new Date()) {
                  const diff = deadlineD - new Date();
                  const dd = Math.floor(diff / 86400000);
                  const hh = Math.floor((diff % 86400000) / 3600000);
                  countdownGuild = dd > 0 ? `${dd}д ${hh}ч` : `${hh}ч`;
                }
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 8 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 9.5, color: '#5a5a6a', marginTop: 1 }}>
                          ⚔️ {boss.condition.label} · ⏳ {countdownGuild || `${daysLeft} дн.`}
                        </div>
                      </div>
                      <div style={{
